...
| +0x2E8 | char | is_falling | Set by Ball_Shatter (via Breaker/Bonkbash crusher objects), Ball_Update (player fall death at 0x406244 & 0x407436), OddBoard_CollisionHandler (0x40F2E7), E:SWALLOW. NOT set by Ball_FallUpdate for player ball (bad-ball-only). Triggers respawn |
| +0x2E9 | byte | death_pending | ⚠ **NOT "reset to 0 each frame"!** Sticky flag that prevents Ball_ApplyTrajectory from re-firing. Set by E:LIMIT, E:LIMITX, E:LIMITZ, E:LIMITPIPE1/2, speed>1.0 collision, and Ball_ApplyTrajectory itself. **NOT set by E:SWALLOW** (which sets +0x2E8 instead). Reset to 0 ONLY by Ball_InitPhysicsDefaults(0x405100) at 0x405262, and Ball_ctor2(0x4039E0) at 0x403BDE. See docs/agent-knowledge/death-pending-flag-deep-dive.md |
| +0x2EC | int32 | bounce_count | Dizzy system bounce counter. Double-incremented (0→1→2) when collision speed exceeds thresholds 0.03 and 0.1. When bounce_count > 1 AND death_pending==0 → Ball_ApplyTrajectory fires. Reset by Ball_DizzyImmunity, Ball_InitPhysicsDefaults, and when has_trajectory(+0x14D) is set |

...
| 0x00403750 | Ball_ApplyTrajectory | The "dizzy" effect: normalize+scale velocity by 0.5, damp Y by 1.25, set impact_count(+0x2F0)=100, set death_pending(+0x2E9)=1, set has_trajectory(+0x14D)=1, create trail particles, play boost sound. If player_index≠-1: increment *(App+pIdx×0xA0+0x5F8) (dizzy counter shown on end screen). Called when bounce_count(+0x2EC) > 1 AND death_pending==0 in Ball_Update Pass 1. |
...
| 0x00405e00 | Ball_Update | 400+ | Main ball physics tick: two-pass collision architecture (Pass 1: check bounce_count>1 → Ball_ApplyTrajectory; Pass 2: increment bounce_count, set death_pending if speed>1.0). Also: stun recovery, timer, velocity integration, reflection, sound, camera tilt, spin. Core function! |

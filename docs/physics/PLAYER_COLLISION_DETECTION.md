...
| `+0x2E9` | `byte` | death_pending | Sticky flag preventing Ball_ApplyTrajectory re-firing (E:LIMIT collision, speed>1.0) — NEVER use for ground check |
| `+0x2EC` | `int32` | bounce_count | Dizzy system bounce counter (double-incremented 0→1→2 when collision speed exceeds 0.03 and 0.1; triggers Ball_ApplyTrajectory when >1 AND death_pending==0) |
...
**Problem**: The `death_pending` at `ball+0x2E9` looks like it indicates

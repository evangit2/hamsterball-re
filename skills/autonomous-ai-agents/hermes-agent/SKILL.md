...
grep -i "telegram\|discord\|error\|unauthorized" ~/.hermes/logs/agent.log | tail -30
grep -i "error" ~/.hermes/logs/errors.log | tail -20
```

Common Telegram gateway failures:

1. **"No user allowlists configured. All unauthorized users will be denied."**
   Edit `~/.hermes/.env` and set:
   ```
   TELEGRAM_ALLOWED_USERS=your_user_id_here
   ```
   Or set `GATEWAY_ALLOW_ALL_USERS=true` (less secure).

2. **"Primary api.telegram.org connection failed" / "Fallback IP also failed"**
   Network can't reach Telegram. Test: `curl -s https://api.telegram.org/`
   Fix: VPN/proxy, DNS fix, or use webhook mode with `TELEGRAM_WEBHOOK_URL`.

3. **Wrong command:** The gateway command is `hermes gateway` (not `hermes telegram`).
   ```bash
   hermes gateway           # Start foreground
   hermes gateway start    # Start as background service
   ```

4. **Bot token not set or wrong:** Check `TELEGRAM_BOT_TOKEN` in `~/.hermes/.env`.
   Verify with: `curl -s "https://api.telegram.org/bot<TOKEN>/getMe" | python3 -m json.tool`

5. **Gateway disconnected but not restarted:** Kill stale processes and start fresh:
   ```bash
   pkill -f "hermes gateway"  # Kill any stale gateway

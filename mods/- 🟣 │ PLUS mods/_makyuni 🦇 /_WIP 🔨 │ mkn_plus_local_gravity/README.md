...
Sets gravity **per level or arena** by reading values from `mkn_plus_local_gravity_set.txt`. Each of the 30 lines corresponds to a specific level (15 races + 15 arenas). The file is re-read on every level start, so changes take effect without restarting the game.
...
The mod auto-creates `mkn_plus_local_gravity_set.txt` next to `Hamsterball.exe` on first launch if it doesn't exist. The file has 30 lines:
...
1. On `onLevelStart`, re-reads `mkn_plus_local_gravity_set.txt`
...
2. **Manual 17-entry vtable** — fixes MinGW/MSVC ABI mismatch (HB+ v2.0)
...
| Config | In-game slider | `mkn_plus_local_gravity_set.txt` file |

...
static void nc_ftoa(double v, char* out, int prec) {
    ...
    for (i = 0; i < prec; i++) pw *= 10U;
    // Integer part (32-bit math only: no 64-bit div helpers in link)
    ip = (unsigned int)v;
    // Round fractional to prec
    frac = v - (double)ip;
    for (i = 0; i < prec; i++) frac *= 10.0;
    fp = (unsigned int)(frac + 0.5);
    // Handle rounding carry (fp == pw means .999.. rounded up)
    if (fp >= pw && pw > 1) { ip += 1; fp = 0; }
    else if (pw == 1) fp = 0;
    {
        char t[32];
        int ti = 0;

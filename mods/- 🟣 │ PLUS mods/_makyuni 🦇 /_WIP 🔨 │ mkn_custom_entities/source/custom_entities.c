/*
 * custom_entities.c — Hamsterball Custom Entities Mod v55m_41
 *
...
static void __thiscall hook_DispatchCollisionEvents(void* this_, int* ball, int* collision_data) {
     ...
     * v55m_41: collision_data[0] is the entity/collision object pointer (matches catapult+0x10D4).
    ...
    if (ai_type == 6) {
        ...
        switch (ai_type) {
            ...
            case 33: /* Bonk_ctor — Warm-Up Race Bonk (5 params: this, board, x, y, z)
                 ...
                 * v55m_41: hook disabled — manual SEH trampoline at DispatchCollisionEvents
            ...
            case 35: /* Catapult_ctor — Stands_ctor family (this, board, mesh)
                ...
                *(float*)((char*)obj + 0x10E0) = pz;
                ...
                /* v55m_41: Do NOT add to board+0x43B8 — causes heap corruption on non-Tower levels.
...
/* v55m_41: Catapult trigger via Present-hook radius check (restored from v55m_28m).
...
static void __cdecl cEnt_catapult_present_check(DWORD board) {
    ...
    for (i = 0; i < g_catapult_count; i++) {
        ...
        if (cs->launching) {
            ...
            /* v55m_41: stronger catapult launch (native Catapult_Launch uses 90.0 velocity) */
            ...
            *(float*)(ball + BALL_FORCE_Z) += dzz * launch_horiz;
            ...
            /* v55m_41 (DIAGNOSTIC): sound disabled to confirm whether the ntdll crash
             * is caused by the sound load/play path. If this build does not crash,
             * the sound system is the culprit. */
#if 0
            {
                DWORD board = get_board();
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55m_41 Started ===\n");

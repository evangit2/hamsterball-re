/*
 * custom_entities.c — Hamsterball Custom Entities Mod v55m_42
 *
...
/* Forward declarations for v55m_42 BASS sound helpers */
...
static DWORD g_dropin_sample = 0;  /* v55m_42: BASS sample for catapult dropin sound */
...
static void __thiscall hook_DispatchCollisionEvents(void* this_, int* ball, int* collision_data) {
     ...
     * v55m_42: collision_data[0] is the entity/collision object pointer (matches catapult+0x10D4).
    ...
    if (ai_type == 6) {
        ...
        switch (ai_type) {
            ...
            case 33: /* Bonk_ctor — Warm-Up Race Bonk (5 params: this, board, x, y, z)
                 ...
                 * v55m_42: hook disabled — manual SEH trampoline at DispatchCollisionEvents
            ...
            case 35: /* Catapult_ctor — Stands_ctor family (this, board, mesh)
                ...
                *(float*)((char*)obj + 0x10E0) = pz;
                ...
                /* v55m_42: Do NOT add to board+0x43B8 — causes heap corruption on non-Tower levels.
                ...
                if (g_catapult_count < MAX_CATAPULTS) {
                    ...
                    /* v55m_42: load dropin sound via BASS for this catapult */
...
static void cEnt_despawn_all_rotaters(DWORD board, FILE* logf) {
    ...
    /* v55m_42: free BASS dropin sample on level unload to avoid leak */
...
/* v55m_42: load dropin sound via BASS directly. Called once per level when a
 * Catapult is spawned. Tries .ogg, .wav, .mp3 extensions. */
static void cEnt_load_dropin_sample(FILE* logfile) {
    if (g_dropin_sample) return;
    if (!real_BASS_SampleLoad) {
        if (logfile) fprintf(logfile, "CATAPULT: BASS_SampleLoad not available\n");
        return;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (g_dropin_sample) {
            if (logfile) fprintf(logfile, "CATAPULT: loaded dropin sample from %s (handle=0x%08lX)\n", path, g_dropin_sample);
            return;
        }
    }
    if (logfile) fprintf(logfile, "CATAPULT: failed to load dropin sample from %s\\sounds\\dropin\n", g_game_dir);
}

/* v55m_42: play the loaded dropin sound via BASS directly. */
static void cEnt_play_dropin_sound(FILE* logfile) {
    FILE* lf = logfile;
    ...
    if (!real_BASS_SampleGetChannel || !real_BASS_ChannelPlay) {
        if (lf) fprintf(lf, "CATAPULT: BASS channel functions not available\n");
        if (lf != logfile) fclose(lf);
        return;
    }
    DWORD ch = real_BASS_SampleGetChannel(g_dropin_sample, FALSE);
    if (lf) fprintf(lf, "CATAPULT: got sample channel 0x%08lX\n", ch);
    if (ch) {
        real_BASS_ChannelPlay(ch, FALSE);
        if (lf) fprintf(lf, "CATAPULT: played dropin sound via BASS\n");
    }
    if (lf != logfile) fclose(lf);
}
...
/* v55m_42: Catapult trigger via Present-hook radius check (restored from v55m_28m).
...
static void __cdecl cEnt_catapult_present_check(DWORD board) {
    ...
    for (i = 0; i < g_catapult_count; i++) {
        ...
        if (cs->launching) {
            ...
            /* v55m_42: stronger catapult launch (native Catapult_Launch uses 90.0 velocity) */
            ...
            *(float*)(ball + BALL_FORCE_Z) += dzz * launch_horiz;
            ...
            /* v55m_42: play dropin sound via BASS directly. This bypasses the game's
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55m_42 Started ===\n");

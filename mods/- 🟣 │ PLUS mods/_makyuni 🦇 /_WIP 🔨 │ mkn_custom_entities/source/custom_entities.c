/*
 * custom_entities.c — Hamsterball Custom Entities Mod v55m_42bb
 *
...
/* Forward declarations for v55m_42b BASS sound helpers */
...
static DWORD g_dropin_sample = 0;  /* v55m_42b: BASS sample for catapult dropin sound */
...
static void __thiscall hook_DispatchCollisionEvents(void* this_, int* ball, int* collision_data) {
     ...
     * v55m_42b: collision_data[0] is the entity/collision object pointer (matches catapult+0x10D4).
    ...
    if (ai_type == 6) {
        ...
        switch (ai_type) {
            ...
            case 33: /* Bonk_ctor — Warm-Up Race Bonk (5 params: this, board, x, y, z)
                 ...
                 * v55m_42b: hook disabled — manual SEH trampoline at DispatchCollisionEvents
            ...
            case 35: /* Catapult_ctor — Stands_ctor family (this, board, mesh)
                ...
                *(float*)((char*)obj + 0x10E0) = pz;
                ...
                /* v55m_42b: Do NOT add to board+0x43B8 — causes heap corruption on non-Tower levels.
                ...
                if (g_catapult_count < MAX_CATAPULTS) {
                    ...
                    /* v55m_42b: load dropin sound via BASS for this catapult */
...
static void cEnt_despawn_all_rotaters(DWORD board, FILE* logf) {
    ...
    /* v55m_42b: free BASS dropin sample on level unload to avoid leak */
...
/* v55m_42bb: load dropin sound via BASS directly. Called once per level when a
...
/* v55m_42bb: play the loaded dropin sound via BASS directly. */
static void cEnt_play_dropin_sound(FILE* logfile) {
    FILE* lf = logfile;
    if (!lf) lf = fopen("custom_entities_catapult.log", "a");
    if (!g_dropin_sample) {
...
/* v55m_42b: Catapult trigger via Present-hook radius check (restored from v55m_28m).
...
static void __cdecl cEnt_catapult_present_check(DWORD board) {
    ...
    for (i = 0; i < g_catapult_count; i++) {
        ...
        if (cs->launching) {
            ...
            /* v55m_42b: stronger catapult launch (native Catapult_Launch uses 90.0 velocity) */
            ...
            *(float*)(ball + BALL_FORCE_Z) += dzz * launch_horiz;
            ...
            /* v55m_42b: play dropin sound via BASS directly. This bypasses the game's
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55m_42b Started ===\n");

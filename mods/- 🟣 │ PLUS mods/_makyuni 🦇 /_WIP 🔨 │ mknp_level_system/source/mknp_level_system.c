/*
 * mknp_level_system v5 — Extended Vtables + Custom Dispatch
 *
 * v4 FIX (retained):
 ...
 *
 * Build:
 *   i686-w64-mingw32-gcc -shared -o bass.dll mknp_level_system.c \
...
static void install_frameupdate_hook(void) {
    ...
    *(DWORD*)(patch_addr + 1) = (DWORD)cave - (DWORD)patch_addr - 5;
    ...
    diag_log("[mknp_level_system] FrameUpdate epilogue hook installed at 0x0046C1F1");
}

/* Summon_NeonEffect — material-only neon glow (D3D8 light BROKEN from slot 36).
 *
 * v6.10 tried creating a D3D8 SceneObject from the dispatch thread (slot 36),
 * but this causes a race condition: the dispatch thread modifies the object list
 * while the render loop iterates it, crashing at 0003:0000044B during Draw.
 *
 * v6.11: Reverted to material-only writes. The D3D8 light cannot be created
 * from the dispatch thread. It would need to run during level setup (native
 * vtable slot), but that also crashes on real Windows during menu navigation.
 *
 * Idempotent: skips if board+0x642C == "NEON" sentinel.
 * Safe for extended slots (36-127). */
static void __fastcall Summon_NeonEffect(DWORD board) {
    ...
    *(DWORD*)(board + 0x642C) = 0x4E454F4E;
    ...
    diag_log("[summon] Summon_NeonEffect: applied neon glow materials (material-only, no light)");
}
...
static void find_config_path(void) {
    ...
    if (p) {
        strcpy(p + 1, "mknp_level_system.log");
    } else {
        strcpy(g_logPath, "mknp_level_system.log");
    }

    p = strrchr(g_configPath, '\\');
    if (p) {
        strcpy(p + 1, "mknp_level_system.txt");
    } else {
        strcpy(g_configPath, "mknp_level_system.txt");
    }
}
...
static void apply_config(void) {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        diag_log("[mknp_level_system v4] Config file not found, no changes applied");
        return;
    }
    ...
    diag_log("========================================");
    diag_log("[mknp_level_system v4] Config loaded, parsing...");
...
/* ============================================================
 * Custom script functions — read from mknp_level_functions.txt
 *
 * Syntax:
 *   FUNCTION MyFuncName
...
/* Load custom functions from mknp_level_functions.txt */
static void load_custom_functions(void) {
    char func_path[MAX_PATH];
    strcpy(func_path, g_configPath);
    char* p = strrchr(func_path, '\\');
    if (p) {
        strcpy(p + 1, "mknp_level_functions.txt");
    } else {
        strcpy(func_path, "mknp_level_functions.txt");
    }

    diag_logf("[mknp_level_system v5] Looking for custom functions: %s", func_path);

    HANDLE hFile = CreateFileA(func_path,
        GENERIC_READ, FILE_SHARE_READ, NULL,
        OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hFile == INVALID_HANDLE_VALUE) {
        diag_log("[mknp_level_system v5] No mknp_level_functions.txt found (custom functions disabled)");
        return;
    }
    ...
    diag_logf("[mknp_level_system v5] Loading custom functions from %s", func_path);
    ...
    diag_logf("[mknp_level_system v5] Loaded %d custom functions", g_num_custom_funcs);
}
...
static void mknp_init(void) {
    find_config_path();

    /* Step 1: Allocate custom vtables and patch ctors */
    init_custom_vtables();
    if (!g_vtables_initialized) {
        diag_log("[mknp_level_system v5] FATAL: custom vtable init failed!");
        return;
    }
    diag_log("[mknp_level_system v5] Custom vtables allocated (128 entries each) and ctors patched");

    /* Step 2: Patch ALL level allocations to max struct size */
    patch_all_struct_sizes();
    diag_logf("[mknp_level_system v5] All 15 levels patched to struct size 0x%X", MAX_STRUCT_SIZE);

    /* Step 3: Load custom script functions from mknp_level_functions.txt */
    ...
    diag_log("[mknp_level_system v5] Extended dispatch thread started (slots 36-127)");
}

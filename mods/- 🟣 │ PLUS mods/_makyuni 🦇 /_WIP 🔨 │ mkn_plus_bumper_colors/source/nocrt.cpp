...
// Touches each page to commit stack memory. Simple implementation:
// just probe the final address — Windows commits the pages automatically.
extern "C" void __chkstk_ms(void) {
    // The stack pointer adjustment is already done by the compiler.
    // We just need to touch the pages to ensure they're committed.
    // Windows handles stack growth automatically via guard pages,
    // so this can be a no-op on Windows.
}

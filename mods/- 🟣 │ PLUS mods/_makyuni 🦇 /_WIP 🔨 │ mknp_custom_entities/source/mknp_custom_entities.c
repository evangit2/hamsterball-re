/*
 * mknp_custom_entities.c — Hamsterball Custom Entities Mod v55n_84
 *
...
/* ScoreObject_ctor (0x44BE80) — __thiscall(this, board, score_slot_ptr, "EXTRA TIME:")
 * Creates the floating "EXTRA TIME:" popup that native UpRaceCollisionEvents
 * appends to board+0x8b8 after a TimeButton press. This is what the player
 * actually SEES on press (the +500 timer write is otherwise invisible).
 *   this->+4 = board, this->+8 = score slot, this->+0xc = App,
 *   +0x14=200000, +0x1c=200, +0x10=0, +0x18/0x28/0x29/0x2a=0, +0x19=1,
 *   +0x2c = label string "EXTRA TIME:". vtable = 0x4D6C70. */
typedef int (__thiscall *ScoreObject_ctor_t)(void* this_, DWORD board, DWORD score_slot, const char* label);
static ScoreObject_ctor_t pfn_ScoreObject_ctor = (ScoreObject_ctor_t)0x0044BE80;

/* Timer_Decrement (0x44BEF0) — __fastcall(obj): Dies at Timer_Decrement after
 * the popup is created (native: Timer_Decrement(iVar2); AthenaList_Append(...)).
 * Sets obj+0x10 -= 100, obj+0x2a = 1. We skip it for the popup path because
 * it's a death timer the ctor already manages; appending is enough. */
...
typedef struct {
    ...
    float yaw;         /* v55n_84: fixed world orientation (ROT_Y) from ref point.
    ...
    /* v55n_84: render-only trigger box PopCylinder (from
    ...
    DWORD trig_obj;        /* render-only trigger box object */
    DWORD cyl_orig_vt18;   /* original vtable[18] of col_level (Level render) */
    DWORD trig_orig_vt18;  /* original vtable[18] of trig_obj (PopCylinder render) */
} SpeedCylState;
...
static int g_speedcyl_heartbeat = 0; /* v55n_48: throttled log counter */
...
/* v55n_38: forward decl — translate collision geometry (defined after the render hook). */
static int cEnt_translate_collision_strips(DWORD coll_level, float dx, float dy, float dz, FILE* logf);

/* v55n_38: shared log path (line 1105 in v55n_8). Declared here so the early
 * render hook can log. */
...
typedef struct {
    ...
    int   geom_translated; /* v55n_38: 1 = collision tree translated to spawn pos */
    DWORD orig_vtable18; /* v55n_5: saved original vtable[18] (0x45E0E0) for render hook */
    /* v55n_38: saved coll-Level mw+0x18 tree originals — the PROVEN catapult
...
static void __thiscall cEnt_timebutton_render(DWORD this_, char param_1, int param_2) {
    ...
    /* v55n_38: geometry translation moved OUT of this hook into the Present
...
/* v55n_38+: Translate a collision Level's collision geometry so the ball hits
...
static int cEnt_translate_collision_strips(DWORD coll_level, float dx, float dy, float dz, FILE* logf) {
     ...
     * every time it ran (v55n_8 at ctor, v55n_38 at Update 0001:0004717E).
...
/* v55n_38: TimeButton collision-tree translate — the PROVEN catapult pattern.
 ...
 * (v55n_38's mistake -> 0001:000570A4) — it writes the built Level's tree from
...
/* v55n_38: Translate a loaded MeshWorld's VERTEX SOURCE by (dx,dy,dz) BEFORE
...
static int cEnt_translate_meshworld_verts(DWORD mw, float dx, float dy, float dz, FILE* logf) {
    ...
    /* ═══ v55n_38: +0x18 collision TREE FIRST, UNCONDITIONALLY ═══
     ...
     * sets +0x18=15 count but the strips are lazy). v55n_38's bug: it checked
    ...
    /* ═══ v55n_38/15: +0x2C MeshBuffer/strip list (belt-and-suspenders) ═══
...
/* v55n_48: Native collision->track handoff = Pendulum_PlayCollisionSound (0x436B70).
...
/* v55n_84: SpeedCylinder visual Y-rotation render hook.
 ...
 * trigger zone. Uses the proven catapult/waterwheel world-matrix rotate-around-
 * center pattern, but the rotation is around Y (not X), with the fixed yaw read
 * from the cEnt ref point (deg→rad). g_in_draw_phase guards the D3D read so it
...
/* v55n_84: The SpeedCylinder render-hook AND the spawn-time world-matrix bake
 * are REMOVED. v55n_84 still crashed on race restart at 0x4526A0 even with no
 * hook, so the crash is caused by writing the cylinder's renderLevel+0x4
 * (obj+0x438) world matrix — from the background entity_thread at spawn (v55n_84
...
/* (g_log_path moved to top, v55n_38) */
...
static DWORD g_spawned_level = 0;  /* v55n_38: also track level so a REUSED board (tournament) re-spawns */

/* Config: seconds between GRID switches */
static float g_grid_speed = 3.0f;
static int g_enable_speedcylinder = 1;  /* v55n_84: config A/B toggle for restart-crash isolation */
...
static void cEnt_Spawn_cEntity_at(DWORD board, float px, float py, float pz,
    ...
    /* v55n_84: Config A/B toggle to isolate the restart crash. When
     ...
     * the SpeedCylinder spawn/registration path is the culprit; if it
     * persists with NOTHING but this mod's other entities active, the bug
     * is elsewhere. Default 1 (speedcylinders ON). */
    if (ai_type == 39 && !g_enable_speedcylinder) {
        if (logf) fprintf(logf, "  cENTITY: SKIPPING SpeedCylinder (enable_speedcylinder=0)\n");
        return;
    }
        ...
        *(float*)(obj + 0x10E8) = 0.0f;
        ...
        switch (ai_type) {
            ...
            case 39: /* SpeedCylinder_ctor — 7 params (this, board, x, y, z, int_param, mesh)
                ...
                *(float*)((char*)obj + 0x10DC) = pz;
                ...
                if (g_speedcyl_count < MAX_SPEEDCYLINDERS) {
                    int sc_idx = g_speedcyl_count;
                    g_speedcyls[sc_idx].obj = (DWORD)obj;
                    g_speedcyls[sc_idx].board = board;
                    g_speedcyls[sc_idx].x = px;
                    g_speedcyls[sc_idx].y = py;
                    g_speedcyls[sc_idx].z = pz;
                    g_speedcyls[sc_idx].col_level = *(DWORD*)((char*)obj + 0x10E0);
                    ...
                    /* v55n_84: per-cylinder fixed world orientation (ROT_Y). */
                    g_speedcyls[sc_idx].yaw = rot_y;
                    /* v55n_84: NO world-matrix write here. Writing the cylinder's
                     * renderLevel+0x4 (obj+0x438) world matrix — whether per-frame
                     * (v55n_75/76/77 hook) or once at spawn (v55n_84 bake) — causes
                 ...
                 * v55n_84: REMOVED the trigger-box visual entirely. Every attempt
                 * to render it crashed: PopCylinder+scene tree (v55n_81 restart
                 * crash 0x478EDD), manual raw-mesh render (v55n_84 first-load
            ...
            case 45: /* v55n_3: TimeButton_ctor — 5 params (this, board, x, y, z, mesh).
                ...
                /* v55n_38: case 45 uses the PROVEN crash-safe SpeedCylinder_ctor
                 * (0x436A20). Single alloc, no orphaned TIMEBUTTON_SIZE block. */
                /* v55n_38: REMOVED the pre-ctor cEnt_translate_meshworld_verts(mesh,...).
                 * v55n_38 added it and it CRASHED at 0001:000570A4 (EXE, Draw, fcomps
                ...
                /* v55n_38 DECISIVE TEST + candidate fix: use the PROVEN crash-safe
                 * SpeedCylinder_ctor (0x436A20) instead of the broken native
                 * TimeButton_ctor (0x436C10). Evidence chain:
                 *   - Both ctors decompile nearly identical (Stands_ctor + Level_RenderCtor
                 *     collision Level at +0x10E0; NEITHER appends any board list).
                 *   - The ONLY differences: vtable (0x4D5830 vs 0x4D57D0) + few field inits.
                 *   - v55n_22 (ZERO registration, native TimeButton_ctor) crashed ntdll 0x4717E.
                 *   - v55n_38 (swapped to SpeedCylinder MESH, still native TimeButton_ctor)
                 *     crash MOVED to exe 0x4527F1. -> mesh shifts WHICH block breaks, so the
                 *     culprit is the TimeButton_ctor path itself.
                 *   - SpeedCylinder_ctor is PROVEN crash-free + solid on ANY level (case 39).
                 * So: allocate a SpeedCylinder-sized obj, run SpeedCylinder_ctor, register it
                 * exactly like case 39, feed it the LevelUp-Button mesh, and track it as a
                 * TimeButton so the mod-side N:EXTRATIME press handler finds it.
                 ...
                 * so the per-frame Update uses TimeButton slots (RenderOnce 0x43DC40)
                 * instead of SpeedCylinder's continuous rotation (slot 11 = 0x43D8C0).
                 * Construction is still via the PROVEN Stable SpeedCylinder_ctor; only the
                 * behavior pointer changes. obj is 0x150C bytes >= TimeButton's 0x10E8,
                 * so every TimeButton vtable slot reads in-bounds.
                 * Also restore the native TimeButton ctor's state flags that
                 * SpeedCylinder_ctor overwrote: +0x10E5=1 (render-once), +0x10E4=0
                 * (not-yet-pressed latch). SpeedCylinder wrote +0x10E4 as the speed float
                 * (0 here) but the byte is 0, which our press handler reads as unpressed —
                 * good. */
                *(DWORD*)obj = 0x4D5830;
                ...
                *(float*)((char*)obj + 0x10DC) = pz;
                {
                    /* 1. Collision Level at obj+0x10E0 (same as case 39) */
                    DWORD tb_col = *(DWORD*)((char*)obj + 0x10E0);
                    if (tb_col && !IsBadReadPtr((void*)tb_col, 0x20)) {
                        /* 2. Set MeshBuffer+0x47C = entity on ALL MeshBuffers (case-39 offsets) */
                        ...
                        /* 3. Register into board+0x10EC + scene tree (proven case-39) */
                ...
                /* 5. Track for N:EXTRATIME collision dispatch (mod-side press handler) */
                if (g_timebutton_count < MAX_TIMEBUTTONS) {
                    ...
                    g_timebuttons[g_timebutton_count].orig_vtable18 = 0; /* no private vtable hook */
                    g_timebutton_count++;
                }
                break;
...
static void cEnt_Despawn_All_cEntities(DWORD board, FILE* logf) {
    ...
    /* v55n_84: SpeedCylinder cleanup — restore the private vtable[18] copies.
     ...
     * before the game's teardown frees those objects, the engine (or a stale
     * board list) calls our hook on the FREED object on the NEXT level restart
     * -> the hook reads freed this_+0x434 -> return-address corruption -> EIP
     * lands mid-instruction in an unrelated Draw fn (crash 0x4526A0 on Warm-Up
     * restart). Mirror the proven waterwheel block (v55m_44o): restore the
     * private vtable's slot 18 to the original, then let the engine teardown
     * use the real functions. The private vtable memory itself is ours; the
     * engine frees the OBJECT but we must neutralise the pointer first. */
    {
        int si;
        for (si = 0; si < g_speedcyl_count; si++) {
            /* a) cylinder collision/render Level vtable restore */
            DWORD col_lvl = g_speedcyls[si].col_level;
            if (col_lvl && col_lvl > 0x10000 && g_speedcyls[si].cyl_orig_vt18) {
                DWORD priv = *(DWORD*)col_lvl;
                if (priv && priv > 0x400000 && !IsBadReadPtr((void*)priv, 0x400) &&
                    !IsBadReadPtr((void*)col_lvl, 4)) {
                    ((DWORD*)priv)[18] = g_speedcyls[si].cyl_orig_vt18;
                }
            }
            /* b) trigger box vtable restore */
            DWORD trig = g_speedcyls[si].trig_obj;
            if (trig && trig > 0x10000 && g_speedcyls[si].trig_orig_vt18) {
                DWORD priv = *(DWORD*)trig;
                if (priv && priv > 0x400000 && !IsBadReadPtr((void*)priv, 0x400) &&
                    !IsBadReadPtr((void*)trig, 4)) {
                    ((DWORD*)priv)[18] = g_speedcyls[si].trig_orig_vt18;
                }
            }
        }
        if (logf) fprintf(logf, "  cENTITY: restored %d SpeedCylinder private vtable[18](s)\n", g_speedcyl_count);
    }
...
/* v55n_48: Forward decl — SpeedCylinder collision->track handoff helper (main-thread Present hook). */
...
/* v55n_38: TimeButton proximity-press helper — runs on MAIN THREAD via the
 * Present hook (same safe pattern as catapult/speedcyl/waterwheel). The
 * N:EXTRATIME DispatchCollisionEvents hook (install_bonk_collision_hook) is
 * NEVER installed in the current build AND its manual trampoline on that SEH
 * function crashes (v53g-2 lesson). So the press is replicated here by
 * proximity instead of waiting for a collision event.
 *
 * Native press (UpRaceCollisionEvents 0x4119B0 + Rotator_TriggerSound
 * 0x436CF0): on press, if +0x10E4==0:
 *   +0x10E4 = 1 (latch), +0x10E5 = 1 (render pressed pose),
 *   +0x10D8 -= 20.0f (sink 20 units), press sound (sound list +0x510),
 *   single-player (profile+0x10==0 && profile+0x11==0):
 *     timer slot (player_idx * 0xA0 + 0x5EC + App) = 500.
 * Press trigger: ball center within ~30 units of the button (button is a
 * wide low slab; a horizontal-radius + vertical-window test mirrors the
 * catapult trigger calibration). Once pressed, the latch stays 1 forever
 * (native buttons are one-shot per level).
 */
#define TIMEBUTTON_PRESS_RADIUS_SQ  1600.0f    /* 40 units horizontal (v55n_38) */
#define TIMEBUTTON_PRESS_DY_MIN     -80.0f     /* v55n_48: tightened from ±160 — ball must be near the button, not just over it */
#define TIMEBUTTON_PRESS_DY_MAX      80.0f

static void __cdecl cEnt_timebutton_present_press(DWORD board) {
    int i;
    static int tb_heartbeat = 0;
    tb_heartbeat++;
    int tb_log_now = ((tb_heartbeat % 30) == 0);
    (void)tb_log_now;

    FILE* df = NULL;   /* v55n_38: NO per-frame fopen/fclose of tb.log (see below) */
    /* v55n_38: We previously opened mknp_custom_entities_tb.log with
     * fopen(...,"a") every 30 frames from inside the Present hook code cave,
     * and the press branch also wrote the main log while the background
     * (entity) thread writes the same shared logf FILE* concurrently.
     * v55n_38 (helper not called at all -> zero file I/O) was STABLE, while
     * v55n_31 (press log-only, same fopens) CRASHED. Prime suspects: repeated
     * fopen/fclose handle churn and/or concurrent writes to shared logf
     * corrupting the C runtime FILE state -> ntdll heap trip in Draw later.
     * v55n_38 removes ALL tb.log file I/O entirely (df stays NULL) and fflushes
     * the main log on press so the press line is never lost to a buffered
     * crash. If v55n_38 is stable with the press firing, file I/O was the
     * crash; if it still crashes, we've narrowed it to the shared-logf write. */
    (void)df;

    DWORD ball = get_ball_ptr();
    if (!ball || ball < 0x10000 || IsBadReadPtr((void*)ball, 0x200)) {
        if (df) { fprintf(df, "TB: %d ball=0x%08X (no ball)\n", tb_heartbeat, ball); fclose(df); }
        return;
    }
    if (!board || board < 0x10000 || IsBadReadPtr((void*)(board + 0x878), 0x20)) {
        if (df) { fprintf(df, "TB: %d board=0x%08X (bad board)\n", tb_heartbeat, board); fclose(df); }
        return;
    }
    if (*(BYTE*)(board + 0x874) != 0) {   /* paused */
        if (df) { fprintf(df, "TB: %d PAUSED (skip)\n", tb_heartbeat); fclose(df); }
        return;
    }
    if (g_timebutton_count <= 0) {
        if (df) { fprintf(df, "TB: %d count=%d (no timebuttons tracked)\n", tb_heartbeat, g_timebutton_count); fclose(df); }
        return;
    }
    ...
    if (df) fprintf(df, "TB: %d ball=(%.1f,%.1f,%.1f) count=%d\n", tb_heartbeat, ball_x, ball_y, ball_z, g_timebutton_count);

    for (i = 0; i < g_timebutton_count; i++) {
        TimeButtonState* tb = &g_timebuttons[i];
        if (!tb || !tb->obj || tb->obj < 0x10000) {
            if (df) fprintf(df, "TB:   [%d] bad tb (obj=0x%08X)\n", i, tb ? tb->obj : 0);
            continue;
        }
        if (tb->board != board) {
            if (df) fprintf(df, "TB:   [%d] tb=0x%08X WRONG BOARD (tb_board=0x%08X board=0x%08X)\n", i, tb->obj, tb->board, board);
            continue;
        }
        if (IsBadReadPtr((void*)tb->obj, 0x10E8)) {
            if (df) fprintf(df, "TB:   [%d] tb=0x%08X bad read\n", i, tb->obj);
            continue;
        }

        float dx = ball_x - tb->x;
        float dy = ball_y - tb->y;
        float dz = ball_z - tb->z;
        float horiz_sq = dx*dx + dz*dz;
        int latched = *(char*)((char*)tb->obj + 0x10E4);

        if (df) fprintf(df, "TB:   [%d] tb=0x%08X pos=(%.1f,%.1f,%.1f) pressed=%d latch=%d dx=%.1f dy=%.1f dz=%.1f horiz=%.1f (r=%d)\n",
            i, tb->obj, tb->x, tb->y, tb->z, tb->pressed, latched, dx, dy, dz, (float)_hypot(dx, dz), TIMEBUTTON_PRESS_RADIUS_SQ);

        if (latched != 0) {
            if (df) fprintf(df, "TB:   [%d] ALREADY PRESSED (latch=%d)\n", i, latched);
            continue;
        }
        if (horiz_sq > TIMEBUTTON_PRESS_RADIUS_SQ) {
            if (df) fprintf(df, "TB:   [%d] too far horizontally (need horiz<=%.0f, have %.1f)\n", i, _hypot(0,TIMEBUTTON_PRESS_RADIUS_SQ), _hypot(dx,dz));
            continue;
        }
        if (dy < TIMEBUTTON_PRESS_DY_MIN || dy > TIMEBUTTON_PRESS_DY_MAX) {
            if (df) fprintf(df, "TB:   [%d] vertical window miss (dy=%.1f need [%.0f,%.0f])\n", i, dy, TIMEBUTTON_PRESS_DY_MIN, TIMEBUTTON_PRESS_DY_MAX);
            continue;
        }

        /* PRESS! — v55n_38 decisive isolation: NO +0x10E4 latch write.
         * v55n_38 (latch + reward, NO sound/sink/pose) crashed IDENTICALLY
         * at ntdll 0x3F0D5. So sound/sink/pose are ALL exonerated (sound was
         * fully #if 0'd). The only remaining press-time writes were the latch
         * +0x10E4=1 and the reward. Writing +0x10E4=1 engages the NATIVE
         * TimeButton state machine (vtable 0x4D5830 slot 11 = 0x43DC40) into
         * its pressed state, which runs during Draw on a SpeedCylinder-built
         * object lacking TimeButton's real internals -> heap corruption.
         * v55n_38 therefore does NOT touch +0x10E4 at all: press = reward +
         * mod-side pressed flag + unconditional log, leaving the entity in
         * its never-pressed (safe) state. If the crash stops, the latch is
         * confirmed as the sole culprit. */
        /* v55n_38: FULL native press behavior (crash root cause = shared-logf
         * race, now fixed in v55n_38; these game-memory writes were never the
         * culprit). v55n_38 proved the press fires cleanly with no crash.
         * Now re-enable the real TimeButton behavior, matching native
         * UpRaceCollisionEvents (0x4119B0) + the vtable 0x4D5830 pressed state:
         *   +0x10E4 = 1 (latch), +0x10E5 = 1 (rendered pressed pose),
         *   +0x10D8 -= 20.0f (sink the button 20 units),
         *   press sound (native channel: app+0x510), and
         *   single-player: timer slot (player_idx*0xA0+0x5EC+App) = +500.
         * Keep ALL logging confined to the thread-private press.log so the
         * Shared-logf race that crashed the game never returns. */
        tb->pressed = 1;   /* mod-side flag (obj+0x10E4 latch is the game-side one) */
        {
            /* latch + pressed pose + sink */
            *(char*)((char*)tb->obj + 0x10E4) = 1;
            *(char*)((char*)tb->obj + 0x10E5) = 1;
            *(float*)((char*)tb->obj + 0x10D8) -= 20.0f;

            /* press log (thread-private) */
            FILE* plf = fopen("mknp_custom_entities_press.log", "a");
            if (plf) { fprintf(plf, "PRESSED v55n_38 entity=0x%08X count=%d heart=%d latch=1 sink+pose set\n", tb->obj, g_timebutton_count, tb_heartbeat); fclose(plf); }
        }

        /* Press sound — native channel = App + 0x510 (proven by Rotator_TriggerSound
         * 0x436CF0 decompile; v55n_29's App+0x4A8 was the chomper channel, wrong).
         * Re-enabled v55n_38 now that the crash source is fixed. */
        {
            DWORD app = *(DWORD*)((char*)board + 0x878);  /* BOARD_APP */
            if (app && app > 0x10000 && !IsBadReadPtr((void*)app, 0x4C0)) {
                DWORD ch = *(DWORD*)((char*)app + 0x510);
                if (ch && ch > 0x10000 && !IsBadReadPtr((void*)ch, 0x20) && pfn_Sound_Play3D) {
                    pfn_Sound_Play3D((void*)ch,
                        *(float*)((char*)tb->obj + 0x10D4),
                        *(float*)((char*)tb->obj + 0x10D8),
                        *(float*)((char*)tb->obj + 0x10DC), 1.0f);
                }
            }
        }

        /* EXTRA TIME reward — v55n_38 root-cause fix (see below).
         * The visible race clock / HUD countdown is NOT App+PID*0xA0+0x5EC.
         * Per Board_UpdateRaceState (0x41B080): board+0x362C holds per-player
         * timer POINTERS (App+PID*0xA0+0x5CC); it DECREMENTS the field at
         * ptr+0x1C = App+PID*0xA0+0x5E8 every frame, and that +0x5E8 is what
         * the HUD displays. The +0x5EC field is only a game-over GATE
         * (checked <1), never shown. Native UpRaceCollisionEvents writes
         * +0x5EC=500 AND pops a visible "EXTRA TIME:" ScoreObject — the
         * popup is the ONLY visible feedback.
         *   v55n_35/36 wrote +0x5EC=500 with no popup -> nothing visible,
         * which is exactly why "still not getting the extra time".
         * v55n_38 (this fix):
         *   1) ADD 500 to the countdown slot App+PID*0xA0+0x5E8 (the real clock).
         *   2) Spawn the native "EXTRA TIME:" ScoreObject popup (ScoreObject_ctor
         *      0x44BE80) and append it to the reward list board+0x8b8 (native
         *      parity) so the player SEES the reward.
         * No single-player gate: reward works in Warm-Up, Time Trial, and
         * Tournament (user request). We only guard pointer validity. */
        {
            DWORD app_tb = *(DWORD*)((char*)board + 0x878);
            if (app_tb && app_tb > 0x10000 && !IsBadReadPtr((void*)app_tb, 0x600)) {
                int player_idx = *(int*)((DWORD)ball + 0x18); /* ball+6 = player index */
                if (player_idx < 0 || player_idx >= 6) player_idx = 0;
                DWORD base = player_idx * 0xA0 + app_tb;
                DWORD countdown_slot = base + 0x5E8;  /* the HUD-visible clock */
                DWORD bonus_slot     = base + 0x5EC;  /* native bonus field */
                if (IsBadReadPtr((void*)countdown_slot, 8)) {
                    /* fall back to player 0 slot */
                    player_idx = 0;
                    base = app_tb;
                    countdown_slot = base + 0x5E8;
                    bonus_slot     = base + 0x5EC;
                }
                if (!IsBadReadPtr((void*)countdown_slot, 8)) {
                    /* 1) Bump the visible clock by +500 sec */
                    int cur = *(int*)countdown_slot;
                    *(int*)countdown_slot = cur + 500;
                    /* 2) native bonus field too (parity) */
                    *(int*)bonus_slot = 500;
                    /* 3) Spawn the visible "EXTRA TIME:" ScoreObject popup.
                     * Popup render (0x44C160) reads *(param_2 + 0x20) and
                     * /100 — so param_2 MUST be the timer BASE (+0x5CC),
                     * making +0x20 land on the +0x5EC bonus (native parity).
                     * Passing bonus_slot (+0x5EC) made it read +0x60C garbage
                     * -> "+0". v55n_38 fix. Native (UpRaceCollisionEvents
                     * 0x4119B0) also calls Timer_Decrement (0x44BEF0) right
                     * after the ctor so the popup auto-dies after 100 ticks. */
                    if (pfn_ScoreObject_ctor && pfn_AthenaList_Append &&
                        !IsBadReadPtr((void*)((char*)board + 0x8b8), 0x20)) {
                        void* sobj = pfn_operator_new(0x30);
                        if (sobj) {
                            int sobj_ret = pfn_ScoreObject_ctor(sobj, board, base + 0x5CC, "EXTRA TIME:");
                            /* Timer_Decrement(0x44BEF0): +0x10 = +0x1C-100; +0x2A=1 */
                            if (sobj_ret) {
                                *(int*)((char*)sobj_ret + 0x10) = *(int*)((char*)sobj_ret + 0x1C) - 100;
                                *(char*)((char*)sobj_ret + 0x2A) = 1;
                            }
                            pfn_AthenaList_Append((DWORD*)((char*)board + 0x8b8), sobj);
                        }
                    }
                    FILE* plf = fopen("mknp_custom_entities_press.log", "a");
                    if (plf) {
                        fprintf(plf, "REWARD +500 granted to player %d (countdown_slot=0x%08X old=%d new=%d popup=EXTRA TIME:)\n",
                            player_idx, countdown_slot, cur, cur + 500);
                        fclose(plf);
                    }
                }
            }
        ...
        /* v55n_38: REMOVED the stray fprintf(logf,...) that followed this
         * block (it was the SECOND shared-logf write in the press branch).
         * The press already logs to the dedicated thread-private press.log.
         * The background entity thread writes logf every frame with no lock;
         * ANY fprintf(logf,...) from the Present hook races msvcrt FILE buffer
         * pointers -> heap corruption that trips in ntdll later in Draw.
         * This was the root cause of 0001:0003F0D5 firing ~6s after press. */
        break;  /* one press per frame is enough */
    }
    if (df) fclose(df);
}
...
/* v55n_48: SpeedCylinder collision->track handoff (main-thread Present hook).
...
static void __cdecl cEnt_speedcyl_present_check(DWORD board) {
    ...
    for (i = 0; i < g_speedcyl_count; i++) {
         ...
         * v55n_84: ROT_Y is a fixed yaw around the vertical (Y) axis that
        ...
        float by = ball_y;
        if (sc->yaw != 0.0f) {
            float yaw_rad = sc->yaw * 3.14159265f / 180.0f;  /* v55n_84: yaw is degrees (from file) */
        ...
        if (!in_zone) {
            sc->was_in_zone = 0;
            sc->in_list = 0;   /* v55n_48: clear latch when ball leaves zone — allows re-trigger after native hold+launch ends */
        }
    }
}
...
static void __cdecl gluebie_present_helper(void) {
    ...
    /* v55n_38: DISABLED timebutton translate -- it's a NO-OP.
     * User log (v55n_38) showed 22x "TBtx tree count=0 (bad)" -- the built
...
#if 0  /* v55n_38: disabled -- no-op, mirrors SpeedCylinder */
...
#endif /* v55n_38 */
#if 0  /* v55n_38: replaced by the vertex-source translate above (proven pattern) */
    if (board && g_timebutton_count > 0 && *(BYTE*)(board + 0x874) == 0) {
        int ui;
        for (ui = 0; ui < g_timebutton_count; ui++) {
            TimeButtonState* tb = &g_timebuttons[ui];
    ...
    /* v55n_38: TimeButton proximity-press re-ENABLED.
     * v55n_38 (helper not called at all) was STABLE with you dropping on the
     * solid button -> the button+collider are innocent (confirmed twice: v55n_27
     * also safe). The crash is entirely inside the press helper. v55n_31's
     * press wrote the SHARED main-log FILE* from the Present hook while the
     * background entity thread also writes logf with no lock -> racing FILE
     * buffer pointers. v55n_38 removes all tb.log churn and writes the press
    ...
    /* v55n_48: SpeedCylinder collision->track handoff FIRST, then slot 11.
     ...
     * v55n_84: RESTART-CRASH FIX — this driver was the ONLY per-frame loop
     * touching SpeedCylinder entities WITHOUT the `board != board` gate that
     * every stable driver (present_check, catapult, timebutton) has. During a
     * race restart the old board is freed while g_speedcyls[] still holds its
     * stale obj pointers (the counter reset only runs later in the background
     * spawn scan), so 0x43D8C0 wrote to freed collision-Level/board memory ->
     * heap corruption surfacing as the moving restart crash (0x4526A0/0x478EDD).
     * Mirror the present_check guard: skip any entity whose board isn't the
     * CURRENT board, and use the same 0x1510 read guard. */
    if (board && g_speedcyl_count > 0 && *(BYTE*)(board + 0x874) == 0) {
        int si;
        for (si = 0; si < g_speedcyl_count; si++) {
            SpeedCylState* scs = &g_speedcyls[si];
            DWORD sc = scs->obj;
            if (!sc || sc < 0x10000) continue;
            if (scs->board != board) continue;   /* v55n_84: gate to CURRENT board */
            if (IsBadReadPtr((void*)sc, 0x1510)) { scs->obj = 0; continue; }
            if (pfn_SpeedCyl_slot11) {
...
static void __cdecl cEnt_draw_text_helper(void) {
    ...
    if (!get_board()) {
        if (g_table_visible) {
            cEnt_draw_text_double(font, "Custom Entities Mod v55n_84", 20, 12,
                                  1.0f, 1.0f, 1.0f, 0.9f);
        }
        return;
    }
...
static void cEnt_Treesearch_cEntities(DWORD board, FILE* logf) {
    ...
    /* v55n_84: Reset ALL per-level cEnt tracking counters at the start of a
     ...
     * entries pointing at freed memory -> crash at 0x4526A0
     * (AthenaList_GetCount) during Draw. Resetting here makes each level
     * start from a clean slate; new spawns repopulate from index 0. */
    g_catapult_count = 0;
    g_speedcyl_count = 0;
    g_timebutton_count = 0;
    g_chomper_count = 0;
    g_bridgeslam_count = 0;
    g_tarbubble_count = 0;
    g_waterwheel_count = 0;
    ...
    for (i = 0; i < obj_count; i++) {
        ...
        /* v55n_84: Per-entity fixed world orientation. Read the Y-rotation
         ...
         * +0x10 (rot.x), +0x14 (rot.y), +0x18 (rot.z) are engine-ordered.
         * The engine Y rotation = the cylinder's yaw around world Y. */
        float ent_rot_y = *(float*)(obj_ptr + 0x14);
        ...
        static const ai_entry_t ai_list[] = {
            ...
            { "Timebutton",       45, "levels\\LevelUp-Button" },    /* v55n_38: restores the BUTTON mesh. Uses proven SpeedCylinder_ctor (0x436A20) so the mesh path matters: if stable -> TimeButton_ctor was the culprit; if crashes -> button mesh itself is broken on non-race levels. */
        ...
        cEnt_Spawn_cEntity_at(board, px, py, pz, ai_mesh,
                         /* v55n_84: use the ref point's ROT_Y as the cylinder's
...
static void load_config(void) {
    ...
    /* v55n_84: Auto-generate the config file next to the DLL if it doesn't
     ...
     * the same values). */
    FILE* gencfg = NULL;
    fopen_s(&gencfg, config_path, "r");
    if (!gencfg) {
        FILE* w = NULL;
        fopen_s(&w, config_path, "w");
        if (w) {
            fprintf(w, "# Custom Entities Mod configuration\n");
            fprintf(w, "# Create or edit this file to change settings.\n");
            fprintf(w, "\n");
            fprintf(w, "# How long each GRID point stays visible before cycling (seconds)\n");
            fprintf(w, "grid_speed = 3.0\n");
            fprintf(w, "\n");
            fprintf(w, "# Set to 0 to disable SpeedCylinder spawning entirely.\n");
            fprintf(w, "# Used to isolate the restart-crash (A/B test).\n");
            fprintf(w, "enable_speedcylinder = 1\n");
            fclose(w);
            FILE* cfglog = NULL;
            char cfglog_path[MAX_PATH];
            snprintf(cfglog_path, MAX_PATH, "%s\\mknp_custom_entities.log", g_game_dir);
            fopen_s(&cfglog, cfglog_path, "a");
            if (cfglog) {
                fprintf(cfglog, "  CONFIG: created mknp_custom_entities.txt with defaults\n");
                fclose(cfglog);
            }
        }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        /* v55n_84: Parse enable_speedcylinder (A/B isolation toggle) */
        if (strncmp(p, "enable_speedcylinder", 20) == 0) {
            char* eq = strchr(p, '=');
            if (eq) {
                int val = atoi(eq + 1);
                g_enable_speedcylinder = (val != 0);
            }
        }
    }
    fclose(f);
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55n_84 Started ===\n");
    ...
    while (g_running) {
        ...
        /* v55n_38: read level here (before the gate) so the gate can compare
         * BOTH board and level. Board pointers are reused across tournament
         * races, so keying on board alone made a same-board new race look
         * already-spawned -> button missing on some tournament loads. */
        DWORD cur_level = cEnt_get_level(board);

        /* Check if board changed (new level loaded).
         * v55n_38: also compare g_spawned_level — board pointers are REUSED
         * across tournament races, so a same-board new race must re-spawn. */
        if (board == g_spawned_board && cur_level == g_spawned_level) {

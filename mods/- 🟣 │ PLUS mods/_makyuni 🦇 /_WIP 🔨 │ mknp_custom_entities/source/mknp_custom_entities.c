/*
/*
 * mknp_custom_entities.c — Hamsterball Custom Entities Mod v55n_45
 *
...
typedef struct {
    ...
    int   was_in_zone; /* v55n_45: edge-detect for collision->track handoff */
    int   in_list;     /* v55n_45: 1 = ball already appended to +0x10F0 */
} SpeedCylState;
static SpeedCylState g_speedcyls[MAX_SPEEDCYLINDERS];
static int g_speedcyl_count = 0;
static int g_speedcyl_heartbeat = 0; /* v55n_45: throttled log counter */
...
/* v55n_45: Native collision->track handoff = Pendulum_PlayCollisionSound (0x436B70).
            ...
            case 39: /* SpeedCylinder_ctor — 7 params (this, board, x, y, z, int_param, mesh)
                ...
                *(float*)((char*)obj + 0x10DC) = pz;
                ...
                if (g_speedcyl_count < MAX_SPEEDCYLINDERS) {
                    ...
                    g_speedcyls[g_speedcyl_count].was_in_zone = 0; /* v55n_45 */
                    g_speedcyls[g_speedcyl_count].in_list = 0;     /* v55n_45 */
                    g_speedcyl_count++;
                }
                break;
...
/* v55n_45: Forward decl — SpeedCylinder collision->track handoff helper (main-thread Present hook). */
...
#define TIMEBUTTON_PRESS_DY_MIN     -80.0f     /* v55n_45: tightened from ±160 — ball must be near the button, not just over it */
...
/* v55n_45: SpeedCylinder collision->track handoff (main-thread Present hook).
...
static void __cdecl cEnt_speedcyl_present_check(DWORD board) {
    ...
    for (i = 0; i < g_speedcyl_count; i++) {
        ...
        if (!in_zone) {
            sc->was_in_zone = 0;
            sc->in_list = 0;   /* v55n_45: clear latch when ball leaves zone — allows re-trigger after native hold+launch ends */
        }
    }
}
...
static void __cdecl gluebie_present_helper(void) {
    ...
    /* v55n_45: SpeedCylinder collision->track handoff FIRST, then slot 11.
...
static void __cdecl cEnt_draw_text_helper(void) {
    ...
    if (!get_board()) {
        if (g_table_visible) {
            cEnt_draw_text_double(font, "Custom Entities Mod v55n_45", 20, 12,
                                  1.0f, 1.0f, 1.0f, 0.9f);
        }
        return;
    }
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55n_45 Started ===\n");

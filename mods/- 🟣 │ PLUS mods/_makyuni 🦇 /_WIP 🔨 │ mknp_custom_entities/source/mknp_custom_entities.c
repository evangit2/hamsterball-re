/*
/*
 * mknp_custom_entities.c — Hamsterball Custom Entities Mod v55n_42
 *
...
typedef struct {
    ...
    int   was_in_zone; /* v55n_42: edge-detect for collision->track handoff */
    int   in_list;     /* v55n_42: 1 = ball already appended to +0x10F0 */
} SpeedCylState;
static SpeedCylState g_speedcyls[MAX_SPEEDCYLINDERS];
static int g_speedcyl_count = 0;
static int g_speedcyl_heartbeat = 0; /* v55n_42: throttled log counter */
...
/* v55n_42: Native collision->track handoff = Pendulum_PlayCollisionSound (0x436B70).
            ...
            case 39: /* SpeedCylinder_ctor — 7 params (this, board, x, y, z, int_param, mesh)
                ...
                *(float*)((char*)obj + 0x10DC) = pz;
                ...
                if (g_speedcyl_count < MAX_SPEEDCYLINDERS) {
                    ...
                    g_speedcyls[g_speedcyl_count].was_in_zone = 0; /* v55n_42 */
                    g_speedcyls[g_speedcyl_count].in_list = 0;     /* v55n_42 */
                    g_speedcyl_count++;
                }
                break;
...
/* v55n_42: Forward decl — SpeedCylinder collision->track handoff helper (main-thread Present hook). */
...
#define TIMEBUTTON_PRESS_DY_MIN     -80.0f     /* v55n_42: tightened from ±160 — ball must be near the button, not just over it */
...
/* v55n_42: SpeedCylinder collision->track handoff (main-thread Present hook).
...
static void __cdecl cEnt_speedcyl_present_check(DWORD board) {
    ...
    for (i = 0; i < g_speedcyl_count; i++) {
        ...
        float horiz_sq = dx*dx + dz*dz;
        int in_zone = (horiz_sq < 7656.25f && dy > -56.25f && dy < 56.25f); /* r=70, h=90 — cylinder r=30 + ball r=26 = 56 contact, margin to 70 */
        ...
        if (!in_zone) {
            sc->was_in_zone = 0;
            sc->in_list = 0;   /* v55n_42: clear latch when ball leaves zone — allows re-trigger after native hold+launch ends */
        }
    }
}
...
static void __cdecl gluebie_present_helper(void) {
    ...
    /* v55n_42: SpeedCylinder collision->track handoff FIRST, then slot 11.
...
static void __cdecl cEnt_draw_text_helper(void) {
    ...
    if (!get_board()) {
        if (g_table_visible) {
            cEnt_draw_text_double(font, "Custom Entities Mod v55n_42", 20, 12,
                                  1.0f, 1.0f, 1.0f, 0.9f);
        }
        return;
    }
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55n_42 Started ===\n");

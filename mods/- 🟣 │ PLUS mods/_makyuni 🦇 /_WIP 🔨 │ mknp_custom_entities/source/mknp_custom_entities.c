/*
/*
 * mknp_custom_entities.c — Hamsterball Custom Entities Mod v55n_23
 *
...
/* v55n_23: forward decl — translate collision geometry (defined after the render hook). */
static int cEnt_translate_collision_strips(DWORD coll_level, float dx, float dy, float dz, FILE* logf);

/* v55n_23: shared log path (line 1105 in v55n_8). Declared here so the early
 * render hook can log. */
...
typedef struct {
    ...
    int   geom_translated; /* v55n_23: 1 = collision tree translated to spawn pos */
    DWORD orig_vtable18; /* v55n_5: saved original vtable[18] (0x45E0E0) for render hook */
    /* v55n_23: saved coll-Level mw+0x18 tree originals — the PROVEN catapult
...
static void __thiscall cEnt_timebutton_render(DWORD this_, char param_1, int param_2) {
    ...
    /* v55n_23: geometry translation moved OUT of this hook into the Present
...
/* v55n_23+: Translate a collision Level's collision geometry so the ball hits
...
static int cEnt_translate_collision_strips(DWORD coll_level, float dx, float dy, float dz, FILE* logf) {
    /* v55n_23>: granular diagnostics — log EVERY early-return so a level-start
     * test log reveals exactly which offset/check fails (v55n_23 showed no
    ...
    /* v55n_23: REMOVED the collision TREE-ITEM translation here. It crashed
     * every time it ran (v55n_8 at ctor, v55n_23 at Update 0001:0004717E).
...
/* v55n_23: TimeButton collision-tree translate — the PROVEN catapult pattern.
 ...
 * (v55n_23's mistake -> 0001:000570A4) — it writes the built Level's tree from
...
/* v55n_23: Translate a loaded MeshWorld's VERTEX SOURCE by (dx,dy,dz) BEFORE
...
static int cEnt_translate_meshworld_verts(DWORD mw, float dx, float dy, float dz, FILE* logf) {
    ...
    if (!mw || mw < 0x10000 || IsBadReadPtr((void*)mw, 0x460)) { if(logf) fprintf(logf,"  ROTATER:   TBtx fail: bad mw\n"); return 0; }
    int total = 0;
    int translated_something = 0;  /* v55n_23: any tree OR strip translated */

    /* ═══ v55n_23: +0x18 collision TREE FIRST, UNCONDITIONALLY ═══
     ...
     * sets +0x18=15 count but the strips are lazy). v55n_23's bug: it checked
    ...
    /* ═══ v55n_23/15: +0x2C MeshBuffer/strip list (belt-and-suspenders) ═══
...
/* (g_log_path moved to top, v55n_23) */
        ...
        *(float*)(obj + 0x10E8) = 0.0f;
        ...
        switch (ai_type) {
            ...
            case 45: /* v55n_3: TimeButton_ctor — 5 params (this, board, x, y, z, mesh).
                ...
                /* v55n_23: REMOVED the pre-ctor cEnt_translate_meshworld_verts(mesh,...).
                 * v55n_23 added it and it CRASHED at 0001:000570A4 (EXE, Draw, fcomps
                ...
                /* v55n_23 ISOLATION TEST: ZERO registration.
                ...
                *(float*)((char*)obj + 0x10D8) = py;
                *(float*)((char*)obj + 0x10DC) = pz;
                /* v55n_23: registration block REMOVED for isolation. */
                ...
                if (g_timebutton_count < MAX_TIMEBUTTONS) {
                    ...
                    g_timebuttons[g_timebutton_count].orig_vtable18 = 0; /* v55n_23: no private vtable hook */
                    g_timebutton_count++;
                }
                break;
...
static void __cdecl gluebie_present_helper(void) {
    ...
    /* v55n_23: DISABLED timebutton translate -- it's a NO-OP.
     * User log (v55n_23) showed 22x "TBtx tree count=0 (bad)" -- the built
...
#if 0  /* v55n_23: disabled -- no-op, mirrors SpeedCylinder */
...
#endif /* v55n_23 */
#if 0  /* v55n_23: replaced by the vertex-source translate above (proven pattern) */
    if (board && g_timebutton_count > 0 && *(BYTE*)(board + 0x874) == 0) {
        int ui;
        for (ui = 0; ui < g_timebutton_count; ui++) {
            TimeButtonState* tb = &g_timebuttons[ui];
...
static void __cdecl cEnt_draw_text_helper(void) {
    ...
    if (!get_board()) {
        if (g_table_visible) {
            cEnt_draw_text_double(font, "Custom Entities Mod v55n_23", 20, 12,
                                  1.0f, 1.0f, 1.0f, 0.9f);
        }
        return;
    }
...
static void process_rotaters(DWORD board, FILE* logf) {
    ...
    for (i = 0; i < obj_count; i++) {
        ...
        static const ai_entry_t ai_list[] = {
            ...
            { "Timebutton",       45, "levels\\LevelUp-SpeedCylinder" },    /* v55n_23 ISOLATION: swapped to WORKING SpeedCylinder mesh. TimeButton_ctor(0x436C10,0x10E8) keeps its own ctor+vtable; only the mesh differs. If it STOPS crashing at ntdll 0x4717E, the LevelUp-Button geometry is the culprit; if it still crashes, the ctor/vtable path is. */
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55n_23 Started ===\n");

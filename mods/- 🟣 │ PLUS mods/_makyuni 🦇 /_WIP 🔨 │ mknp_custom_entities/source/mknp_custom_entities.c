/*
/*
 * mknp_custom_entities.c — Hamsterball Custom Entities Mod v55n_51
 *
...
typedef struct {
    ...
    float center_x, center_y, center_z; /* v55n_48: CYLINDER CENTER world pos = spawn + (72.5,0,0) */
} SpeedCylState;
            ...
            case 39: /* SpeedCylinder_ctor — 7 params (this, board, x, y, z, int_param, mesh)
                ...
                *(float*)((char*)obj + 0x10DC) = pz;
                ...
                if (g_speedcyl_count < MAX_SPEEDCYLINDERS) {
                    ...
                    /* v55n_48: zone center = cylinder center = spawn + half-length (145/2) along +X */
...
static void __cdecl cEnt_speedcyl_present_check(DWORD board) {
    ...
    for (i = 0; i < g_speedcyl_count; i++) {
        ...
        float horiz_sq = dx*dx + dz*dz;
        int in_zone = (horiz_sq < 2601.0f && dy > -23.0f && dy < 20.25f); /* v55n_51: r=51 (51^2=2601) to catch ball touching cylinder surface at horiz~56, Y lower bound -23 (ball rests dy~-21.4), upper +20.25 */
...
static void __cdecl cEnt_draw_text_helper(void) {
    ...
    if (!get_board()) {
        if (g_table_visible) {
            cEnt_draw_text_double(font, "Custom Entities Mod v55n_51", 20, 12,
                                  1.0f, 1.0f, 1.0f, 0.9f);
        }
        return;
    }
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55n_51 Started ===\n");

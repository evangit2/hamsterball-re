/*
/*
 * mknp_custom_entities.c — Hamsterball Custom Entities Mod v55m_43h
 *
...
static void __cdecl cEnt_catapult_present_check(DWORD board) {
    ...
    for (i = 0; i < g_catapult_count; i++) {
        ...
        if (log_now) {
            ...
            if (df) {
                if (rot > 0) {
                    fprintf(df, "CATAPULT: rotated %d collision verts (angle=%.2f)\n",
                        rot, cs->arm_angle);
                } else {
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55m_43h Started ===\n");

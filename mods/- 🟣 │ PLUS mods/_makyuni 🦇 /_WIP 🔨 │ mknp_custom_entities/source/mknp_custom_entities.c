/*
/*
 * mknp_custom_entities.c — Hamsterball Custom Entities Mod v55n_32
 *
...
/* v55n_32: forward decl — translate collision geometry (defined after the render hook). */
static int cEnt_translate_collision_strips(DWORD coll_level, float dx, float dy, float dz, FILE* logf);

/* v55n_32: shared log path (line 1105 in v55n_8). Declared here so the early
 * render hook can log. */
...
typedef struct {
    ...
    int   geom_translated; /* v55n_32: 1 = collision tree translated to spawn pos */
    DWORD orig_vtable18; /* v55n_5: saved original vtable[18] (0x45E0E0) for render hook */
    /* v55n_32: saved coll-Level mw+0x18 tree originals — the PROVEN catapult
...
static void __thiscall cEnt_timebutton_render(DWORD this_, char param_1, int param_2) {
    ...
    /* v55n_32: geometry translation moved OUT of this hook into the Present
...
/* v55n_32+: Translate a collision Level's collision geometry so the ball hits
...
static int cEnt_translate_collision_strips(DWORD coll_level, float dx, float dy, float dz, FILE* logf) {
    /* v55n_32>: granular diagnostics — log EVERY early-return so a level-start
     * test log reveals exactly which offset/check fails (v55n_32 showed no
    ...
    /* v55n_32: REMOVED the collision TREE-ITEM translation here. It crashed
     * every time it ran (v55n_8 at ctor, v55n_32 at Update 0001:0004717E).
...
/* v55n_32: TimeButton collision-tree translate — the PROVEN catapult pattern.
 ...
 * (v55n_32's mistake -> 0001:000570A4) — it writes the built Level's tree from
...
/* v55n_32: Translate a loaded MeshWorld's VERTEX SOURCE by (dx,dy,dz) BEFORE
...
static int cEnt_translate_meshworld_verts(DWORD mw, float dx, float dy, float dz, FILE* logf) {
    ...
    if (!mw || mw < 0x10000 || IsBadReadPtr((void*)mw, 0x460)) { if(logf) fprintf(logf,"  ROTATER:   TBtx fail: bad mw\n"); return 0; }
    int total = 0;
    int translated_something = 0;  /* v55n_32: any tree OR strip translated */

    /* ═══ v55n_32: +0x18 collision TREE FIRST, UNCONDITIONALLY ═══
     ...
     * sets +0x18=15 count but the strips are lazy). v55n_32's bug: it checked
    ...
    /* ═══ v55n_32/15: +0x2C MeshBuffer/strip list (belt-and-suspenders) ═══
...
/* (g_log_path moved to top, v55n_32) */
        ...
        *(float*)(obj + 0x10E8) = 0.0f;
        ...
        switch (ai_type) {
            ...
            case 45: /* v55n_3: TimeButton_ctor — 5 params (this, board, x, y, z, mesh).
                ...
                /* v55n_32: case 45 uses the PROVEN crash-safe SpeedCylinder_ctor
                 * (0x436A20). Single alloc, no orphaned TIMEBUTTON_SIZE block. */
                /* v55n_32: REMOVED the pre-ctor cEnt_translate_meshworld_verts(mesh,...).
                 * v55n_32 added it and it CRASHED at 0001:000570A4 (EXE, Draw, fcomps
                ...
                /* v55n_32 DECISIVE TEST + candidate fix: use the PROVEN crash-safe
                 ...
                 *   - v55n_32 (swapped to SpeedCylinder MESH, still native TimeButton_ctor)
                ...
                memset(obj, 0, SPEEDCYLINDER_SIZE);
                pfn_SpeedCylinder_ctor(obj, (void*)board, px, py, pz, 0, mesh);
                /* v55n_32: Override the vtable to the native TimeButton vtable (0x4D5830)
...
/* v55n_32: Forward decl — TimeButton proximity-press helper (main-thread Present hook). */
...
/* v55n_32: TimeButton proximity-press helper — runs on MAIN THREAD via the
 ...
 */
#define TIMEBUTTON_PRESS_RADIUS_SQ  1600.0f    /* 40 units horizontal (v55n_32) */
#define TIMEBUTTON_PRESS_DY_MIN     -160.0f    /* v55n_32: ball arcs over the button (dy up to ~120) */
...
static void __cdecl cEnt_timebutton_present_press(DWORD board) {
    ...
    FILE* df = NULL;   /* separate file so main log isn't spammed */
    if (tb_log_now) df = fopen("mknp_custom_entities_tb.log", "a");
    ...
    for (i = 0; i < g_timebutton_count; i++) {
        ...
        /* PRESS! — v55n_32 decisive isolation: NO +0x10E4 latch write.
         * v55n_32 (latch + reward, NO sound/sink/pose) crashed IDENTICALLY
         ...
         * v55n_32 therefore does NOT touch +0x10E4 at all: press = reward +
        ...
        /* ALWAYS log the press to the MAIN log (unconditionally; tb log was
         * NULL on the non-30-frame crash frame in v55n_29/v55n_32). */
        if (logf) fprintf(logf, "  ROTATER: TimeButton PRESSED (v55n_32: no latch write, entity=0x%08X, count=%d)\n", tb->obj, g_timebutton_count);
        else {
            FILE* plf = fopen("mknp_custom_entities_tb.log", "a");
            if (plf) { fprintf(plf, "TB: %d **** PRESSED (v55n_32: latch only, no sound/sink/pose) **** entity=0x%08X\n", tb_heartbeat, tb->obj); fclose(plf); }
        }

        /* Press sound — DISABLED v55n_32 (isolation test). Same crash with two
         * channels (v55n_28 +0x510, v55n_32 App+0x4A8) proves sound not guilty,
        ...
        /* Reward (single-player): DISABLED v55n_32 for decisive isolation.
...
static void __cdecl gluebie_present_helper(void) {
    ...
    /* v55n_32: DISABLED timebutton translate -- it's a NO-OP.
     * User log (v55n_32) showed 22x "TBtx tree count=0 (bad)" -- the built
...
#if 0  /* v55n_32: disabled -- no-op, mirrors SpeedCylinder */
...
#endif /* v55n_32 */
#if 0  /* v55n_32: replaced by the vertex-source translate above (proven pattern) */
    if (board && g_timebutton_count > 0 && *(BYTE*)(board + 0x874) == 0) {
        int ui;
        for (ui = 0; ui < g_timebutton_count; ui++) {
            TimeButtonState* tb = &g_timebuttons[ui];
    ...
    /* v55n_32 A/B: TimeButton proximity-press DISABLED (isolate press-vs-collider).
     * v55n_32 stripped the press to log-only (NO game-memory write: no latch,
     * no reward, no sound, no sink, no pose) yet STILL crashed at ntdll 0x3F0D5,
     * and the tb.log heartbeat stopped at frame 720 then died at 18s. Because
     * the main-log PRESSED line is a buffered fprintf (lost on hard crash), its
     * absence doesn't prove the press never fired. Your v55n_27 already proved
     * ball-on-solid-button contact is safe (you fell on it, dizzy, no crash).
     * So the correct A/B is: KEEP the collider solid, but don't call the press
     * helper at all. Stable -> press path is the trigger (narrow inside it);
     * crashes -> pure collision/render on the registered Level (attack that). */
    if (0 && board && g_timebutton_count > 0) {
...
static void __cdecl cEnt_draw_text_helper(void) {
    ...
    if (!get_board()) {
        if (g_table_visible) {
            cEnt_draw_text_double(font, "Custom Entities Mod v55n_32", 20, 12,
                                  1.0f, 1.0f, 1.0f, 0.9f);
        }
        return;
    }
...
static void process_rotaters(DWORD board, FILE* logf) {
    ...
    for (i = 0; i < obj_count; i++) {
        ...
        static const ai_entry_t ai_list[] = {
            ...
            { "Timebutton",       45, "levels\\LevelUp-Button" },    /* v55n_32: restores the BUTTON mesh. Uses proven SpeedCylinder_ctor (0x436A20) so the mesh path matters: if stable -> TimeButton_ctor was the culprit; if crashes -> button mesh itself is broken on non-race levels. */
...
static DWORD WINAPI entity_thread(LPVOID param) {
    ...
    if (logf) {
        fprintf(logf, "=== Custom Entities Mod v55n_32 Started ===\n");

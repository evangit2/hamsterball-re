/* entarea.h -- Woodbridge_area gate zones (v1bz).
 * jsonc (verbatim MAKYUNI schema):
 *     "Woodbridge_area", { "behaviour": "Woodbridge_area", "mesh": "vertices" }
 * mesh "vertices" = load the quad from the VERTICES of the S6 geom(s) whose
 * name contains the def name (Woodbridge_area01/02/...) in the LEVEL file
 * (S1 refs are single points and cannot hold verts; the S1 REF: is only an

/*
 * global-funball — bass.dll proxy
 * Spawn FunBall ball entity
 *
...
static obj_ctor_t pfn_obj_ctor = (obj_ctor_t)0x40AFE0;
...
static void* load_mesh(void) {
    ...
    if (!mem) return NULL;
    g_mesh = pfn_meshworld_ctor(mem, gfx, "Meshes\\FunBall");
    return g_mesh;
}
...
static void spawn_at_player(void) {
    ...
    void* mem = pfn_operator_new(0xC98);
    if (!mem) return;

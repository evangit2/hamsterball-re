 ...
 *   mov [playerZ], eax
 * 
 *   cmp dword ptr [SpawnWeakball], 0
 *   je original_code
 *   cmp dword ptr [esi+0x18], 0
 *   jne original_code
 * 
 *   mov dword ptr [SpawnWeakball], 0
 ...
 * 
 *   mov dword ptr [ebx+0x188], 0x40000000   // master speed = 2.0
 *   mov eax, [ebx+0x1A4]
 *   test eax, eax
 *   jz no_new
 *   mov dword ptr [eax+0xC70], 0x40800000   // max speed = 4.0
 *   mov dword ptr [eax+0xC68], 0x3F800000
 * 
 * no_new:
 ...
 * 
 * unregistersymbol(SpawnWeakball)
 * dealloc(SpawnCode)
 * dealloc(spawnHook)
 * dealloc(SpawnWeakball)
 * dealloc(playerX)
 * dealloc(playerY)
 * dealloc(playerZ)
 * dealloc(ballCount)

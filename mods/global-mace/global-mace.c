/*
 * global-mace — bass.dll proxy
 * Spawns Mace at player position
 *
...
static obj_ctor_t pfn_obj_ctor = (obj_ctor_t)0x438750;
...
static void* load_mesh(void) {
    ...
    if (!mem) return NULL;
    g_mesh = pfn_meshworld_ctor(mem, gfx, "Levels\\Level4-Mace");
    return g_mesh;
}
...
static void spawn_at_player(void) {
    ...
    void* mem = pfn_operator_new(0x110C);
    if (!mem) return;

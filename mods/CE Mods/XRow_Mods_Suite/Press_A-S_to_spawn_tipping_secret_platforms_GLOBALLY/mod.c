 ...
 * [ENABLE]
 * // ============================================================
 * //  SpawnTipSecret - Pillar Constructor + Secret Meshes
 * //  Hotkey 1: Secret-Unlock (Y-33)
 * //  Hotkey 2: Secret (Y-33)
 * // ============================================================
 * 
 * alloc(SpawnCode, 4096)
 * alloc(SpawnTipSecret, 4)
 * alloc(SpawnTipSecret2, 4)
 * alloc(saved_board, 4)
 * alloc(playerX, 4)
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 ...
 * alloc(temp_mesh, 4)
 * alloc(mesh_select, 4)
 * alloc(tipsecret_array, 36)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnTipSecret)
 * registersymbol(SpawnTipSecret2)
 * 
 * SpawnTipSecret:
 *   dd 0
 * SpawnTipSecret2:
 ...
 *   cmp edx, 8
 *   jae clear_done
 *   mov eax, [tipsecret_array+4+edx*4]
 *   test eax, eax
 *   jz clear_next
 ...
 *   jmp clear_loop
 * clear_done:
 *   mov dword [tipsecret_array], 0
 *   pop eax
 *   mov [last_board], eax
 * 
 * no_clear:
 *   mov eax, [tipsecret_array]
 *   test eax, eax
 *   jz save_pos
 *   pushad
 *   xor ebx, ebx
 * 
 * prox_loop:
 *   cmp ebx, [tipsecret_array]
 *   jae prox_done
 *   mov eax, [tipsecret_array+4+ebx*4]
 *   test eax, eax
 *   jz prox_next
 *   mov ecx, [eax]
 *   test ecx, ecx
 ...
 * 
 * check_s1:
 *   cmp dword [SpawnTipSecret], 0
 *   je check_s2
 *   cmp dword [esi+0x18], 0
 *   jne check_s2
 *   mov dword [SpawnTipSecret], 0
 ...
 * 
 * check_s2:
 *   cmp dword [SpawnTipSecret2], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnTipSecret2], 0
 ...
 *   cmp eax, 8
 *   jae done_spawn
 *   mov [tipsecret_array+4+eax*4], edi
 *   inc dword [tipsecret_array]
 * 
 * done_spawn:
 *   popad
 ...
 * 
 * dealloc(SpawnCode)
 * dealloc(SpawnTipSecret)
 * dealloc(SpawnTipSecret2)
 * dealloc(saved_board)
 * dealloc(playerX)
 * dealloc(playerY)
 * dealloc(playerZ)
 ...
 * dealloc(temp_mesh)
 * dealloc(mesh_select)
 * dealloc(tipsecret_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnTipSecret)
 * unregistersymbol(SpawnTipSecret2)
 * 
 * ================================================================
 */

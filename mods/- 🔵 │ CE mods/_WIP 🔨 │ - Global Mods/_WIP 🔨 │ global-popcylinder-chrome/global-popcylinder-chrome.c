/*
 * global-popcylinder-chrome — bass.dll proxy
 * Chrome (BadBall) Spawner v8 — replicates OddBoard_Update spawning
 *
...
static obj_ctor_t pfn_ctor = (obj_ctor_t)0x405D90;
...
static void spawn_at_player(void) {
    ...
    void* mem = pfn_new(0xC64);
    if (!mem) return;
    
    void* obj = pfn_ctor(mem, (void*)board, x, y, z, mesh);
    if (!obj) return;
    
    /* Init physics via vtable[1] */
    if (!IsBadReadPtr((void*)obj, 4)) {
        DWORD* vtbl = *(DWORD**)obj;
        typedef void (__thiscall *init_phys_t)(void* obj);
        if (vtbl && !IsBadReadPtr((void*)vtbl, 8)) {
            init_phys_t init = (init_phys_t)vtbl[1];
            if (init) init(obj);
        }
    }
    pfn_append((void*)(board + 0x29D4), (int)obj);
}

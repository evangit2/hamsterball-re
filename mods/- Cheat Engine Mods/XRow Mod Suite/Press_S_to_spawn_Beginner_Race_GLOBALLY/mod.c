 ...
 * [ENABLE]
 * // ============================================================
 * //  Global Beginner Race Spawner (Cascade)
 * //  Spawns LevelCascade geometry at Player 1 position
 * // ============================================================
 * 
 * alloc(SpawnCode, 4096)
 * alloc(SpawnBeginnerRace, 4)
 * alloc(saved_board, 4)
 * alloc(playerX, 4)
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 * alloc(float_400_sub, 4)
 * alloc(float_200_sub, 4)
 * alloc(temp_mesh, 4)
 * alloc(beginner_array, 36)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnBeginnerRace)
 * 
 * SpawnBeginnerRace:
 *   dd 0
 * 
 ...
 *   cmp edx, 8
 *   jae clear_done
 *   mov eax, [beginner_array+4+edx*4]
 *   test eax, eax
 *   jz clear_next
 ...
 * 
 *   fld dword [esi+0x164]
 *   fadd dword [float_200_sub]
 *   fstp dword [playerX]
 *   fld dword [esi+0x168]
 *   fadd dword [float_400_sub]
 *   fstp dword [playerY]
 *   mov eax, [esi+0x16C]
 *   mov [playerZ], eax
 * 
 * check_spawn:
 *   cmp dword [SpawnBeginnerRace], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnBeginnerRace], 0
 * 
 *   pushad
 *   mov eax, [esi+0x14]
 *   mov [saved_board], eax
 ...
 * 
 * skip_render:
 *   mov eax, [beginner_array]
 *   cmp eax, 8
 *   jae done_spawn
 *   mov [beginner_array+4+eax*4], edi
 *   inc dword [beginner_array]
 * 
 * done_spawn:
 *   popad
 ...
 * 
 * dealloc(SpawnCode)
 * dealloc(SpawnBeginnerRace)
 * dealloc(saved_board)
 * dealloc(playerX)
 * dealloc(playerY)
 * dealloc(playerZ)
 * dealloc(float_400_sub)
 * dealloc(float_200_sub)
 * dealloc(temp_mesh)
 * dealloc(beginner_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnBeginnerRace)
 * 
 * ================================================================
 */

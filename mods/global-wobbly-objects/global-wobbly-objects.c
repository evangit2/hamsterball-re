/*
 * global-wobbly-objects — bass.dll proxy
 * Multi: WAVY1, WOBBLY1-4
 *
...
static obj_ctor_t pfn_obj_ctor = (obj_ctor_t)0x43AD40;
...
static void* load_mesh(void) {
    ...
    if (!mem) return NULL;
    g_mesh = pfn_meshworld_ctor(mem, gfx, "Levels\\Level7-Wobbly1");
    return g_mesh;
}
...
static void spawn_at_player(void) {
    ...
    void* mem = pfn_operator_new(0x1524);
    if (!mem) return;

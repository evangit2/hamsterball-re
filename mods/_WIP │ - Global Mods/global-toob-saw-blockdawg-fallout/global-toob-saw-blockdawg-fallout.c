/*
 * global-toob-saw-blockdawg-fallout — bass.dll proxy
 * Multi: SPINNY, SAW, etc.
 *
...
static obj_ctor_t pfn_obj_ctor = (obj_ctor_t)0x435940;
...
static void* load_mesh(void) {
    ...
    if (!mem) return NULL;
    g_mesh = pfn_meshworld_ctor(mem, gfx, "Levels\\Level8-Spinny");
    return g_mesh;
}
...
static void spawn_at_player(void) {
    ...
    void* mem = pfn_operator_new(0x1508);
    if (!mem) return;

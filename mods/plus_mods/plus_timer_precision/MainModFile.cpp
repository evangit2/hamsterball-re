...
private:
    ...
    // Saved original bytes for restore
    static unsigned char s_origCompute[NUM_PATCHES][27];
    static unsigned char s_origFmtPush[NUM_PATCHES][5];
    static bool s_origSaved;

    static void saveOriginals() {
        if (s_origSaved) return;
        for (int i = 0; i < NUM_PATCHES; i++) {
            const PatchSite& p = s_patches[i];
            memcpy(s_origCompute[i], (void*)p.computeAddr, 27);
            memcpy(s_origFmtPush[i], (void*)p.fmtPushAddr, 5);
        }
        s_origSaved = true;
    }
    ...
    static void restoreOriginals(IModAPI* api) {
        for (int i = 0; i < NUM_PATCHES; i++) {
            const PatchSite& p = s_patches[i];
            api->PatchMemory(p.fmtPushAddr, (const char*)s_origFmtPush[i], 5);
            api->PatchMemory(p.computeAddr, (const char*)s_origCompute[i], 27);
        }
    }
...
public:
    ...
    void onButtonToggle(const char* id, bool state) override {
        ...
        if (state) applyPatches(api);
        else restoreOriginals(api);
    }
};
...
unsigned char TimerPrecision::s_origCompute[NUM_PATCHES][27];
unsigned char TimerPrecision::s_origFmtPush[NUM_PATCHES][5];
bool TimerPrecision::s_origSaved = false;

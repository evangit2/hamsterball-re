/*
 * global-sky-objects — bass.dll proxy
 * Multi: FUNBALL, PILLAR, MAGNIFYER
 *
...
static obj_ctor_t pfn_obj_ctor = (obj_ctor_t)0x436250;
...
__declspec(dllexport) volatile DWORD g_object_type = 1;
...
static void* load_mesh(void) {
    ...
    if (!mem) return NULL;
    g_mesh = pfn_meshworld_ctor(mem, gfx, "levels\\level9-popcylinder1");
    return g_mesh;
}
...
static void spawn_at_player(void) {
    ...
    void* mem = pfn_operator_new(0x10F4);
    if (!mem) return;

 ...
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 * alloc(button_array, 36)
 * alloc(float_3600, 4)
 * alloc(float_20, 4)
 * alloc(float_50, 4)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnTimeBtn)
 * 
 * SpawnTimeBtn:
 *   dd 0
 * 
 ...
 *   jmp clear_loop
 * clear_done:
 *   mov dword [button_array], 0
 *   pop eax
 *   mov [last_board], eax
 *   jmp save_pos
 * 
 * no_clear:
 *   mov eax, [button_array]
 *   test eax, eax
 *   jz save_pos
 * 
 *   pushad
 ...
 * 
 * prox_loop:
 *   cmp ebx, [button_array]
 *   jae prox_done
 * 
 *   mov eax, [button_array+4+ebx*4]
 *   test eax, eax
 *   jz prox_next
 * 
 *   mov ecx, [eax]
 ...
 *   mov byte [eax+0x10E5], 1
 *   fld dword [eax+0x10D8]
 *   fsub dword [float_20]
 *   fstp dword [eax+0x10D8]
 *   jmp prox_done
 * 
 * prox_next:
 *   inc ebx
 ...
 * 
 * check_spawn:
 *   cmp dword [SpawnTimeBtn], 0
 *   je original_code
 * 
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 * 
 *   mov dword [SpawnTimeBtn], 0
 * 
 *   pushad
 * 
 *   mov eax, [esi+0x14]
 ...
 *   jz skip_post_spawn
 * 
 *   mov ecx, [button_array]
 *   cmp ecx, 8
 *   jae skip_post_spawn
 *   mov [button_array+4+ecx*4], eax
 *   inc dword [button_array]
 * 
 * skip_post_spawn:
 ...
 * dealloc(playerY)
 * dealloc(playerZ)
 * dealloc(button_array)
 * dealloc(float_3600)
 * dealloc(float_20)
 * dealloc(float_50)
 * dealloc(last_board)
 * unregistersymbol(SpawnTimeBtn)
 * 
 * ================================================================
 */

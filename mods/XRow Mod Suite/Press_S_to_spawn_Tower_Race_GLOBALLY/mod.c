/*
 * "Press S to spawn Tower Race GLOBALLY"
 ...
 * [ENABLE]
 * // ============================================================
 * //  Global Tower Race Spawner
 * //  Spawns Level4 geometry at Player 1 position
 * // ============================================================
 * 
 * alloc(SpawnCode, 4096)
 * alloc(SpawnTowerRace, 4)
 * alloc(saved_board, 4)
 * alloc(playerX, 4)
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 ...
 * alloc(float_500_sub, 4)
 * alloc(temp_mesh, 4)
 * alloc(tower_array, 36)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnTowerRace)
 * 
 * SpawnTowerRace:
 *   dd 0
 * 
 ...
 *   fstp dword [playerY]
 *   fld dword [esi+0x16C]
 *   fadd dword [float_500_sub]
 *   fstp dword [playerZ]
 * 
 * check_spawn:
 *   cmp dword [SpawnTowerRace], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnTowerRace], 0
 * 
 *   pushad
 *   mov eax, [esi+0x14]
 *   mov [saved_board], eax
 ...
 *   cmp eax, 8
 *   jae done_spawn
 *   mov [tower_array+4+eax*4], edi
 *   inc dword [tower_array]
 * 
 * done_spawn:
 *   popad
 ...
 * dealloc(float_500_sub)
 * dealloc(temp_mesh)
 * dealloc(tower_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnTowerRace)
 * 
 * ================================================================
 */

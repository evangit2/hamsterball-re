 ...
 * [ENABLE]
 * // ============================================================
 * //  Global Red Bridge Spawner
 * //  Spawns Level5-Bridge at Player 1 position (Y-15)
 * // ============================================================
 * 
 * alloc(SpawnCode, 4096)
 * alloc(SpawnRedBridge, 4)
 * alloc(saved_board, 4)
 * alloc(playerX, 4)
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 ...
 * alloc(temp_mesh, 4)
 * alloc(redbridge_array, 36)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnRedBridge)
 * 
 * SpawnRedBridge:
 *   dd 0
 * 
 ...
 *   cmp edx, 8
 *   jae clear_done
 *   mov eax, [redbridge_array+4+edx*4]
 *   test eax, eax
 *   jz clear_next
 ...
 * 
 * check_spawn:
 *   cmp dword [SpawnRedBridge], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnRedBridge], 0
 * 
 *   pushad
 *   mov eax, [esi+0x14]
 *   mov [saved_board], eax
 ...
 * 
 * skip_render:
 *   mov eax, [redbridge_array]
 *   cmp eax, 8
 *   jae done_spawn
 *   mov [redbridge_array+4+eax*4], edi
 *   inc dword [redbridge_array]
 * 
 * done_spawn:
 *   popad
 ...
 * dealloc(temp_mesh)
 * dealloc(redbridge_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnRedBridge)
 * 
 * ================================================================
 */

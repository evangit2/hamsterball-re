 ...
 * [ENABLE]
 * // ============================================================
 * //  SpawnRedTip - Pillar Constructor + Red Bridge Mesh
 * // ============================================================
 * 
 ...
 * alloc(float_50, 4)
 * alloc(temp_mesh, 4)
 * alloc(redtip_array, 36)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnRedTip)
 * 
 * SpawnRedTip:
 *   dd 0
 * 
 ...
 *   jmp clear_loop
 * clear_done:
 *   mov dword [redtip_array], 0
 *   pop eax
 *   mov [last_board], eax
 * 
 * no_clear:
 *   mov eax, [redtip_array]
 *   test eax, eax
 *   jz save_pos
 *   pushad
 *   xor ebx, ebx
 * 
 * prox_loop:
 *   cmp ebx, [redtip_array]
 *   jae prox_done
 *   mov eax, [redtip_array+4+ebx*4]
 *   test eax, eax
 *   jz prox_next
 *   mov ecx, [eax]
 *   test ecx, ecx
 ...
 * 
 * check_spawn:
 *   cmp dword [SpawnRedTip], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnRedTip], 0
 * 
 *   pushad
 *   mov eax, [esi+0x14]
 *   mov [saved_board], eax
 ...
 *   cmp eax, 8
 *   jae done_spawn
 *   mov [redtip_array+4+eax*4], edi
 *   inc dword [redtip_array]
 * 
 * done_spawn:
 *   popad
 ...
 * dealloc(float_50)
 * dealloc(temp_mesh)
 * dealloc(redtip_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnRedTip)
 * 
 * ================================================================
 */

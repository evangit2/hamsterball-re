 ...
 * [ENABLE]
 * // ============================================================
 * //  Global Intermediate Race Spawner
 * //  Spawns Level2 geometry at Player 1 position
 * // ============================================================
 * 
 * alloc(SpawnCode, 4096)
 * alloc(SpawnIntermediateRace, 4)
 * alloc(saved_board, 4)
 * alloc(playerX, 4)
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 * alloc(float_800_sub, 4)
 * alloc(float_250_sub, 4)
 * alloc(float_1400_add, 4)
 * alloc(temp_mesh, 4)
 * alloc(intermediate_array, 36)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnIntermediateRace)
 * 
 * SpawnIntermediateRace:
 *   dd 0
 * 
 * saved_board:
 *   dd 0
 ...
 *   cmp edx, 8
 *   jae clear_done
 *   mov eax, [intermediate_array+4+edx*4]
 *   test eax, eax
 *   jz clear_next
 ...
 *   jmp clear_loop
 * clear_done:
 *   mov dword [intermediate_array], 0
 *   pop eax
 *   mov [last_board], eax
 *   jmp save_pos
 * 
 ...
 * 
 *   fld dword [esi+0x164]
 *   fadd dword [float_250_sub]
 *   fstp dword [playerX]
 *   fld dword [esi+0x168]
 *   fadd dword [float_800_sub]
 *   fstp dword [playerY]
 *   fld dword [esi+0x16C]
 *   fadd dword [float_1400_add]
 *   fstp dword [playerZ]
 * 
 * check_spawn:
 *   cmp dword [SpawnIntermediateRace], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnIntermediateRace], 0
 * 
 *   pushad
 *   mov eax, [esi+0x14]
 *   mov [saved_board], eax
 ...
 * 
 * skip_render:
 *   mov eax, [intermediate_array]
 *   cmp eax, 8
 *   jae done_spawn
 *   mov [intermediate_array+4+eax*4], edi
 *   inc dword [intermediate_array]
 * 
 * done_spawn:
 *   popad
 ...
 * 
 * dealloc(SpawnCode)
 * dealloc(SpawnIntermediateRace)
 * dealloc(saved_board)
 * dealloc(playerX)
 * dealloc(playerY)
 * dealloc(playerZ)
 * dealloc(float_800_sub)
 * dealloc(float_250_sub)
 * dealloc(float_1400_add)
 * dealloc(temp_mesh)
 * dealloc(intermediate_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnIntermediateRace)
 * 
 * ================================================================
 */

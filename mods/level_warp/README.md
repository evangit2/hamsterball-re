...
- **v7g**: Fixed timer freeze — was freezing board+0x2990 (static initial time limit, never changes during race) instead of the live timer. The live race timer is board+0x3624 (float, -0.02/frame in Board_UpdateRaceState at 0x41B1BF). Since the mod also uses board+0x3624 for the white screen fade effect, the fix NOPs the FSUB+FST decrement instruction (12 bytes at 0x41B1C5) during the warp, preventing the game from decrementing the timer while the mod writes g_whiteAlpha to the same field. Timer decrement is restored when the warp completes.

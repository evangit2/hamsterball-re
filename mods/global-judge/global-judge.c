/*
 * global-judge — bass.dll proxy
 * Spawns Judge globally at player position
 *
...
static obj_ctor_t pfn_obj_ctor = (obj_ctor_t)0x43A150;
...
static void* load_mesh(void) {
    ...
    if (!mem) return NULL;
    g_mesh = pfn_meshworld_ctor(mem, gfx, "meshes\\hammyjudge");
    return g_mesh;
}
...
static void spawn_at_player(void) {
    ...
    void* mem = pfn_operator_new(0x1100);
    if (!mem) return;
    
    void* obj = pfn_obj_ctor(mem, (void*)board, x, y, z, mesh);
    if (!obj) return;
    
    pfn_append((void*)(board + 0x480), (int)obj);
}

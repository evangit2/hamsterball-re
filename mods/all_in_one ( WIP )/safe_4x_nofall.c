/*
 * safe_4x_nofall.c — Safe BASS.dll proxy: 4x Ball Size + No Fall Damage
 *
 * Combines two mods:
 *   1. 4x Ball Size     — all balls 4x bigger
 *   2. No Fall Damage    — ball never shatters from falling off track
 *
 ...
 *
 * Build:
 *   i686-w64-mingw32-gcc -shared -o bass.dll safe_4x_nofall.c \
...
/* ═══════════════════════════════════════════════════════════════════════════
 * MOD 2: No Fall Damage (3 patches)
 *
 ...
 *   This is the CRITICAL path the previous "unbreakable" mod missed!
 *   When the fall timer (ball+0xC60) drops below threshold, this instruction
 *   sets ball+0x2E8 = 1 (shatter flag) directly — without calling Ball_Shatter.
 *   Original: C6 86 E8 02 00 00 01 (MOV BYTE [ESI+0x2E8], 1) = 7 bytes
...
#define FALL_SHATTER_LEN      7

static int apply_no_fall_damage(BYTE *base)
{
    int results = 0;
    ...
    results += patch_bytes(base + (FALL_SHATTER_ADDR - IMAGE_BASE),
                           (const BYTE*)FALL_SHATTER_ORIGINAL,
                           (const BYTE*)FALL_SHATTER_NOPS, FALL_SHATTER_LEN);
    return results;
}

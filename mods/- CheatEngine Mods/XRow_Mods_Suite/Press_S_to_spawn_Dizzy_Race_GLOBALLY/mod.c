 ...
 * [ENABLE]
 * // ============================================================
 * //  Global Dizzy Race Spawner
 * //  Spawns Level3 geometry at Player 1 position
 * // ============================================================
 * 
 * alloc(SpawnCode, 4096)
 * alloc(SpawnDizzyRace, 4)
 * alloc(saved_board, 4)
 * alloc(playerX, 4)
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 * alloc(float_250, 4)
 * alloc(float_300_sub, 4)
 * alloc(float_400_add, 4)
 * alloc(temp_mesh, 4)
 * alloc(dizzy_array, 36)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnDizzyRace)
 * 
 * SpawnDizzyRace:
 *   dd 0
 * 
 ...
 * 
 *   fld dword [esi+0x164]
 *   fadd dword [float_300_sub]
 *   fstp dword [playerX]
 *   fld dword [esi+0x168]
 *   fadd dword [float_250]
 *   fstp dword [playerY]
 *   fld dword [esi+0x16C]
 *   fadd dword [float_400_add]
 *   fstp dword [playerZ]
 * 
 * check_spawn:
 *   cmp dword [SpawnDizzyRace], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnDizzyRace], 0
 * 
 *   pushad
 *   mov eax, [esi+0x14]
 *   mov [saved_board], eax
 ...
 *   cmp eax, 8
 *   jae done_spawn
 *   mov [dizzy_array+4+eax*4], edi
 *   inc dword [dizzy_array]
 * 
 * done_spawn:
 *   popad
 ...
 * 
 * dealloc(SpawnCode)
 * dealloc(SpawnDizzyRace)
 * dealloc(saved_board)
 * dealloc(playerX)
 * dealloc(playerY)
 * dealloc(playerZ)
 * dealloc(float_250)
 * dealloc(float_300_sub)
 * dealloc(float_400_add)
 * dealloc(temp_mesh)
 * dealloc(dizzy_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnDizzyRace)
 * 
 * ================================================================
 */

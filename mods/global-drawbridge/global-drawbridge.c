/*
 * global-drawbridge — bass.dll proxy
 * Spawns Drawbridge at player position
 *
...
static obj_ctor_t pfn_obj_ctor = (obj_ctor_t)0x4396F0;
...
static void* load_mesh(void) {
    ...
    if (!mem) return NULL;
    g_mesh = pfn_meshworld_ctor(mem, gfx, "Levels\\Level4-Drawbridge");
    return g_mesh;
}
...
static void spawn_at_player(void) {
    ...
    void* mem = pfn_operator_new(0x10FC);
    if (!mem) return;

 ...
 * [ENABLE]
 * // ============================================================
 * //  Global Secret Objects Spawner
 * //  Hotkey 1: Secret-Unlock (Y-33, same XZ)
 * //  Hotkey 2: Secret (Y-33, same XZ)
 * // ============================================================
 * 
 * alloc(SpawnCode, 4096)
 * alloc(SpawnSecret, 4)
 * alloc(SpawnSecret2, 4)
 * alloc(saved_board, 4)
 * alloc(playerX, 4)
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 ...
 * alloc(temp_mesh, 4)
 * alloc(mesh_select, 4)
 * alloc(secret_array, 36)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnSecret)
 * registersymbol(SpawnSecret2)
 * 
 * SpawnSecret:
 *   dd 0
 * SpawnSecret2:
 ...
 * 
 * check_s1:
 *   cmp dword [SpawnSecret], 0
 *   je check_s2
 *   cmp dword [esi+0x18], 0
 *   jne check_s2
 *   mov dword [SpawnSecret], 0
 ...
 * 
 * check_s2:
 *   cmp dword [SpawnSecret2], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnSecret2], 0
 ...
 * 
 * skip_render:
 *   mov eax, [secret_array]
 *   cmp eax, 8
 *   jae done_spawn
 *   mov [secret_array+4+eax*4], edi
 *   inc dword [secret_array]
 * 
 * done_spawn:
 *   popad
 ...
 * dealloc(temp_mesh)
 * dealloc(mesh_select)
 * dealloc(secret_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnSecret)
 * unregistersymbol(SpawnSecret2)
 * 
 * ================================================================
 */

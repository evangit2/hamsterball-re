 ...
 * alloc(playerY, 4)
 * alloc(playerZ, 4)
 * alloc(float_50_sub, 4)
 * alloc(temp_mesh, 4)
 * alloc(wobbly_array, 68)
 * alloc(last_board, 4)
 * 
 * registersymbol(SpawnWobbly)
 * 
 * SpawnWobbly:
 *   dd 0
 * 
 ...
 *   mov [playerX], eax
 *   fld dword [esi+0x168]
 *   fadd dword [float_50_sub]
 *   fstp dword [playerY]
 *   mov eax, [esi+0x16C]
 *   mov [playerZ], eax
 * 
 * check_spawn:
 *   cmp dword [SpawnWobbly], 0
 *   je original_code
 *   cmp dword [esi+0x18], 0
 *   jne original_code
 *   mov dword [SpawnWobbly], 0
 * 
 *   pushad
 *   mov eax, [esi+0x14]
 *   mov [saved_board], eax
 ...
 * dealloc(playerY)
 * dealloc(playerZ)
 * dealloc(float_50_sub)
 * dealloc(temp_mesh)
 * dealloc(wobbly_array)
 * dealloc(last_board)
 * unregistersymbol(SpawnWobbly)
 * 
 * ================================================================
 */

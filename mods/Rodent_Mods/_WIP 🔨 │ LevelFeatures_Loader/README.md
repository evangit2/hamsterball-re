...
- **LevelFeatures.txt deprecated (Option B)**: `[OBJECTS]`/`[FEATURES]`/`[COLLISION]` no longer read. `LoadConfig`/`InitCollisionDefaults`/`LoadCollisionConfig`/`IsCollisionEventEnabled`/`g_defaultFeatures`/`g_featureNames` deleted (26KB saved, 6485 lines). Every `N:`/`E:` S1 ref (`N:BUMPER`, `E:CATAPULTBOTTOM`, `N:GLASS`, `E:BRANCH`, etc.) plus every `*.MESHWORLD`/`*.MESH` in `Levels\` and level folder byte-scanned via `ScanFileForCollisions`/`ScanLevelFolderForCollisions` (CLOUDSCAPE, Level3-WaterWheel hidden sub-meshes, E:SECRET) into per-board `OFF_COLLISION_NAMES` (128×32B at `0xA8D0`→`0xB8D4` in `0xC000` ext) and checked via `IsS1CollisionEnabled` generic prefix (covers `E:BRANCH(A)`, `N:NEONPLATFORM*`). `RaceFiles.txt` remains as the only file (slot→`.MESHWORLD` map).
- **Tower/Sky split (v12.3)**: `REND_WINDMILL (1<<1)` / `REND_CHOMPER (1<<5)` / `REND_TURRET tower (1<<6)` each gated on `REND_TOWER_* !=0` (WINDMILL/CHOMPER/TURRET* S1). `REND_SKY_BOX (1<<3, CLOUDSCAPE)` for far-clip+sprite vs `REND_SKY_LIST (1<<4)` for transparent list (popcylinders). Tower outer gate now `OR` of all three so CHOMPER-only levels render. Ghidra verified 15 `vtable[24]` (9 shared `0x40B420`, 6 customs: Beginner/Toob/Master lifter-highlight `0x411380/0x410670/0x412DC0` + Tower `0x40DFA0` + Glass `0x417C00` + Sky `0x410E80`). `BUMPER`=`N:BUMPER*`, `GLASS`=`N:GLASS` S1-gated.

...
### CreateNoDizzy (0x0040c5d0)

- **Severity:** CRITICAL
- **Suggested name:** `LevelObjectCollisionHandler`
- **Description:** Named 'CreateNoDizzy' after just ONE of ~15 event types it handles. This is actually the GENERAL level object collision/event handler called from 28+ sites. Handles: N:SECRET, N:UNLOCKSECRET, E:NODIZZY, E:SAFESWITCH, E:LIMIT, E:BREAK, E:JUMP, E:ACTION, E:TRAJECTORY, N:NOCONTROL, N:WATER, N:TARPIT, DROPIN, PIPEBONK, POPOUT, N:GOAL, N:MOUSETRAP. Does NOT create anything - it's a collision response handler.
...
### CreateSawblade (0x0040e250)

- **Severity:** MODERATE
- **Suggested name:** `CreateMechanicalObjects2`
- **Description:** Named 'CreateSawblade' but creates 6 different object types: BONK, SLOW/SUPER, UP, SAWBLADE, BRIDGE, and calls Bonk_ctor, TowerLevel_Ctor, Sawblade_Level_Ctor, Spinner_Level_ctor, Gear_Level_ctor, Tipper_Level_Ctor. Should be named after the category of objects it creates, not just one.
- **Evidence:** 6 distinct _ctor calls, handles BONK/SLOW/SUPER/UP/SAWBLADE/BRIDGE/NEG/JUDGE/BELL strings

### CreateSpeedCylinder (0x004117b0)

- **Severity:** MODERATE
- **Suggested name:** `CreateRotatorsAndPendulums`
- **Description:** Named 'CreateSpeedCylinder' but creates Rotator and Pendulum objects. Handles LIFTER, SPEEDCYLINDER, TIMEBUTTON strings but the actual objects created are Rotator_ctor_sound, Rotator_ctor_nosound, and Pendulum_ctor.
- **Evidence:** 3 distinct _ctor calls: Rotator_ctor_sound, Rotator_ctor_nosound, Pendulum_ctor

### CreateSpinner (0x00412850)

- **Severity:** MODERATE
- **Suggested name:** `CreateArenaObjects`
- **Description:** Named 'CreateSpinner' but handles N:SPINNER, N:BUMPER, E:LAUNCH, LAUNCHPOINT, EXPLODEHELPER, E:CALLHAMMER, E:HAMMERCHASE, E:CATAPULTBOTTOM. Only creates RumbleScore_ctor. This is an arena-specific object factory, not just a spinner creator.
